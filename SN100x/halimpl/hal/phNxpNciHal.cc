  ...
  NXPLOG_NCIHAL_E("FW version from device = 0x%x", wFwVerRsp);
    ...
    if (NFCSTATUS_SUCCESS == phNxpNciHal_CheckValidFwVersion()) {
      ...
      if(nfcFL.chipType != sn100u) {
        ...
        ese_gpio_value = phNxpNciHal_ioctl(HAL_NFC_GET_SPM_STATUS, &data);
      }
...
void phNxpNciHal_getVendorConfig(NfcConfig& config) {
  ...
  if (GetNxpNumValue(NAME_DEFAULT_SYS_CODE_PWR_STATE, &num, sizeof(num))) {
    config.defaultSystemCodePowerState = phNxpNciHal_updateAutonomousPwrState(num);
  }
...
void phNxpNciHal_getNxpConfig(nfc_nci_IoctlInOutData_t *pInpOutData) {
  ...
  if (GetNxpNumValue(NAME_FORWARD_FUNCTIONALITY_ENABLE, &num, sizeof(num))) {
    pInpOutData->out.data.nxpConfigs.fwdFunctionality = num;
  }
