...
NCI_LxDebug_Decoder*
NCI_LxDebug_Decoder::mLxDbgDecoder = nullptr;
 ...
 ******************************************************************************/
NCI_LxDebug_Decoder&
NCI_LxDebug_Decoder::getInstance() {
 ...
 **
 ** Description:     This function process Lx NTF, finds the type of it whether L1
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::processLxDbgNciPkt(uint8_t *pNciPkt,
                                        uint16_t pktLen) {
    LOG_FUNCTION_ENTRY;
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::parseL1DbgNtf(psLxNtfCoded_t   psLxNtfCoded,
    ...
    else if(psLxNtfCoded->pLxNtf[2] == L1_EVT_EXTRA_DBG_LEN)    // 0x08 == L1 Event Entry + EDD
    {
        if(mRssiDebugMode)
        {
            decodeCLIFFState(psLxNtfCoded, psLxNtfDecodingInfo, psLxNtfDecoded);
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::parseL2DbgNtf(psLxNtfCoded_t   psLxNtfCoded,
                                   psLxNtfDecoded_t psLxNtfDecoded) {
    LOG_FUNCTION_ENTRY;
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::printLxDebugInfo(psLxNtfDecoded_t psLxNtfDecoded) {

    if(psLxNtfDecoded != nullptr)
    {
        if(psLxNtfDecoded->level == SYSTEM_DEBUG_STATE_L1_MESSAGE)
        {
            phOsal_LogDebug((const uint8_t*)"---------------------L1 Debug Information--------------------");
            phOsal_LogDebugU32dd((const uint8_t*)"Time Stamp", psLxNtfDecoded->psL1NtfDecoded->sInfo.timeStampMs, psLxNtfDecoded->psL1NtfDecoded->sInfo.timeStampUs);
            phOsal_LogDebugString((const uint8_t*)"Trigger Type",psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType);
            phOsal_LogDebugString((const uint8_t*)"RF Tech and Mode",psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode);
            phOsal_LogDebugString((const uint8_t*)"Event Type",psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection);
            if(mRssiDebugMode)
            {
                phOsal_LogDebugU32h((const uint8_t*)"Raw RSSI ADC", psLxNtfDecoded->psL1NtfDecoded->sInfo.rawRSSIADC);
                phOsal_LogDebugU32h((const uint8_t*)"Raw RSSI AGC", psLxNtfDecoded->psL1NtfDecoded->sInfo.rawRSSIAGC);
            }
            else
                phOsal_LogDebugU32hh((const uint8_t*)"Interpolated RSSI", psLxNtfDecoded->psL1NtfDecoded->sInfo.intrpltdRSSI[0], psLxNtfDecoded->psL1NtfDecoded->sInfo.intrpltdRSSI[1]);
            phOsal_LogDebugU32hh((const uint8_t*)"Auto Power Control", psLxNtfDecoded->psL1NtfDecoded->sInfo.APC[0], psLxNtfDecoded->psL1NtfDecoded->sInfo.APC[1]);
            phOsal_LogDebugString((const uint8_t*)"L1 Error EDD",psLxNtfDecoded->psL1NtfDecoded->sInfo.pEddL1Error);
            phOsal_LogDebugString((const uint8_t*)"L1 RxNak EDD",psLxNtfDecoded->psL1NtfDecoded->sInfo.pEddL1RxNak);
            phOsal_LogDebugString((const uint8_t*)"L1 TxErr EDD",psLxNtfDecoded->psL1NtfDecoded->sInfo.pEddL1TxErr);
            phOsal_LogDebugU32hh((const uint8_t*)"L1 7816-4 Ret Code", psLxNtfDecoded->psL1NtfDecoded->sInfo.eddL178164RetCode[0], psLxNtfDecoded->psL1NtfDecoded->sInfo.eddL178164RetCode[1]);
            if((psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection) &&
               (!strcmp((const char*)psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection, (const char*)"CLF_EVT_TX")))
            {
                phOsal_LogDebugU32d((const uint8_t*)"Residual Carrier", psLxNtfDecoded->psL1NtfDecoded->sInfo.residualCarrier);
                phOsal_LogDebugU32d((const uint8_t*)"Number Driver", psLxNtfDecoded->psL1NtfDecoded->sInfo.numDriver);
                phOsal_LogInfo32f((const uint8_t*)"Vtx AMP", psLxNtfDecoded->psL1NtfDecoded->sInfo.vtxAmp);
                phOsal_LogInfo32f((const uint8_t*)"Vtx LDO", psLxNtfDecoded->psL1NtfDecoded->sInfo.vtxLDO);
                phOsal_LogDebugU32d((const uint8_t*)"Tx Vpp", psLxNtfDecoded->psL1NtfDecoded->sInfo.txVpp);
            }
            phOsal_LogDebug((const uint8_t*)"-------------------------------------------------------------");
        }
        else if(psLxNtfDecoded->level == SYSTEM_DEBUG_STATE_L2_MESSAGE)
        {
            uint8_t tlvCount = psLxNtfDecoded->psL2NtfDecoded->tlvCount;

            phOsal_LogDebug((const uint8_t *)"---------------------L2 Debug "
                                            "Information--------------------");
...
/*******************************************************************************
 **
 ** Function:        decodeTimeStamp(psLxNtfCoded_t, psLxNtfDecodingInfo_t, psLxNtfDecoded_t)
 **
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::decodeTimeStamp( psLxNtfCoded_t        psLxNtfCoded,
...
/*******************************************************************************
 **
 ** Function:        decodeRSSIandAPC(psLxNtfCoded_t, psLxNtfDecodingInfo_t, psLxNtfDecoded_t)
 **
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::decodeRSSIValues(psLxNtfCoded_t        psLxNtfCoded,
                                      psLxNtfDecodingInfo_t psLxNtfDecodingInfo,
                                      psLxNtfDecoded_t      psLxNtfDecoded) {
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::decodeAPCTable(psLxNtfCoded_t        psLxNtfCoded,
                                    psLxNtfDecodingInfo_t psLxNtfDecodingInfo,
                                    psLxNtfDecoded_t      psLxNtfDecoded) {
    ...
    if(psLxNtfCoded->pLxNtf[1] == SYSTEM_DEBUG_STATE_L1_MESSAGE)
    {
        ...
        psLxNtfDecoded->psL1NtfDecoded->sInfo.residualCarrier = mLOOKUP_RESCARRIER[(APC[1] & mLOOKUP_RESCARRIER_BITMASK)];
        psLxNtfDecoded->psL1NtfDecoded->sInfo.numDriver = mLOOKUP_NUMDRIVER[(APC[1] & mLOOKUP_NUMDRIVER_BITMASK)];
 ...
 **                  ( (VtxLDO + VtxAmp) * (1 - ((ResCarrier*1.61)/2)/100) ) * (2 - NumDriver + 1);
 **
 ** Returns:         void
 **
 ******************************************************************************/
void
NCI_LxDebug_Decoder::calculateTxVpp(psLxNtfDecoded_t psLxNtfDecoded) {
...
/*******************************************************************************
 **
 ** Function:        decodeCLIFFState(psLxNtfCoded_t, psLxNtfDecodingInfo_t, psLxNtfDecoded_t)
 **
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::decodeCLIFFState(psLxNtfCoded_t        psLxNtfCoded,
                                      psLxNtfDecodingInfo_t psLxNtfDecodingInfo,
                                      psLxNtfDecoded_t      psLxNtfDecoded) {
...
/*******************************************************************************
 **
 ** Function:        decodeTriggerType(psLxNtfCoded_t, psLxNtfDecodingInfo_t, psLxNtfDecoded_t)
 **
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::decodeTriggerType(psLxNtfCoded_t        psLxNtfCoded,
                                       psLxNtfDecodingInfo_t psLxNtfDecodingInfo,
                                       psLxNtfDecoded_t      psLxNtfDecoded) {
    ...
    if(psLxNtfCoded->pLxNtf[1] == SYSTEM_DEBUG_STATE_L1_MESSAGE)
    {
        switch((psLxNtfCoded->pLxNtf[base + offsetTriggerType] & 0x0F))
        {
            case CLF_L1_EVT_ACTIVATED:  //APC
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[1];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[0];
                break;
            case CLF_L1_EVT_DATA_RX:    //RSSI
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[2];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[0];
                break;
            case CLF_L1_EVT_RX_DESLECT: //RSSI
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[3];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[0];
                break;
            case CLF_L1_EVT_RX_WTX:     //RSSI
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[4];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[0];
                break;
            case CLF_L1_EVT_ERROR:            //RSSI, EDD Error Types
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[5];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[0];
                break;
            case CLF_L1_EVT_RX_ACK:            //RSSI
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[6];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[0];
                break;
            case CLF_L1_EVT_RX_NACK:        //RSSI, IOT1, IOT2, IOT3, IOT4, IOT5
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[7];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[0];
                break;
            case CLF_L1_EVT_DATA_TX:        //APC , DPLL
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[8];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[1];
                break;
            case CLF_L1_EVT_WTX_AND_DATA_TX:    //APC, DPLL
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[9];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[1];
                break;
            case CLF_L1_EVT_TX_DESELECT:    //APC, DPLL
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[10];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[1];
                break;
            case CLF_L1_EVT_TX_WTX:            //APC, DPLL
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[11];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[1];
                break;
            case CLF_L1_EVT_TX_ACK:            //APC, DPLL
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[12];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[1];
                break;
            case CLF_L1_EVT_TX_NAK:            //APC, DPLL
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[13];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[1];
                break;
            case CLF_L1_EVT_EXTENDED:        //APC, 7816-4 Return Code
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerType = mCLF_STAT_L1_TRIG_TYPE[14];
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateTriggerTypeDirection = mCLF_EVT_DIRECTION[1];
                break;
            default:
                break;
...
/*******************************************************************************
 **
 ** Function:        decodeRFTechMode(psLxNtfCoded_t, psLxNtfDecodingInfo_t, psLxNtfDecoded_t)
 **
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::decodeRFTechMode(psLxNtfCoded_t        psLxNtfCoded,
                                      psLxNtfDecodingInfo_t psLxNtfDecodingInfo,
                                      psLxNtfDecoded_t      psLxNtfDecoded) {
    ...
    if(psLxNtfCoded->pLxNtf[1] == SYSTEM_DEBUG_STATE_L1_MESSAGE)
    {
        switch((psLxNtfCoded->pLxNtf[base + offsetRFTechMode] & 0xF0))
        {
            case CLF_STATE_TECH_CE_A:    //APC for Tx Events
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[1];
                break;
            case CLF_STATE_TECH_CE_B:    //APC for Tx Events
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[2];
                break;
            case CLF_STATE_TECH_CE_F:    //APC for Tx Events
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[3];
                break;
            case CLF_STATE_TECH_NFCIP1_TARGET_PASSIVE_A:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[4];
                break;
            case CLF_STATE_TECH_NFCIP1_TARGET_PASSIVE_F:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[5];
                break;
            case CLF_STATE_TECH_NFCIP1_TARGET_ACTIVE_A:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[6];
                break;
            case CLF_STATE_TECH_NFCIP1_TARGET_ACTIVE_F:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[7];
                break;
            case CLF_STATE_TECH_RM_A:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[8];
                break;
            case CLF_STATE_TECH_RM_B:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[9];
                break;
            case CLF_STATE_TECH_RM_F:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[10];
                break;
            case CLF_STATE_TECH_NFCIP1_INITIATOR_PASSIVE_A:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[11];
                break;
            case CLF_STATE_TECH_NFCIP1_INITIATOR_PASSIVE_B:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[12];
                break;
            case CLF_STATE_TECH_NFCIP1_INITIATOR_PASSIVE_F:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[13];
                break;
            default:
                psLxNtfDecoded->psL1NtfDecoded->sInfo.pCliffStateRFTechNMode = mCLF_STAT_RF_TECH_MODE[0];
                break;
        }
    }
}
...
/*******************************************************************************
 **
 ** Function:        decodeExtraDbgData(psLxNtfCoded_t, psLxNtfDecodingInfo_t, psLxNtfDecoded_t)
 **
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::decodeExtraDbgData(psLxNtfCoded_t        psLxNtfCoded,
                                        psLxNtfDecodingInfo_t psLxNtfDecodingInfo,
                                        psLxNtfDecoded_t      psLxNtfDecoded) {
...
/*******************************************************************************
 **
 ** Function:        decode78164RetCode(psLxNtfCoded_t, psLxNtfDecodingInfo_t, psLxNtfDecoded_t)
 **
 ...
 ******************************************************************************/
void
NCI_LxDebug_Decoder::decode78164RetCode(psLxNtfCoded_t        psLxNtfCoded,
                                        psLxNtfDecodingInfo_t psLxNtfDecodingInfo,
                                        psLxNtfDecoded_t      psLxNtfDecoded) {
