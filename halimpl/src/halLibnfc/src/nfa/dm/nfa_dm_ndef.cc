...
bool nfa_dm_ndef_dereg_hdlr(tNFA_DM_MSG *p_data) {
  ...
  if (((p_dereginfo->ndef_type_handle & NFA_HANDLE_GROUP_MASK) !=
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("Invalid handle for NDEF type handler: 0x%08x",
...
nfa_dm_ndef_find_next_handler(tNFA_DM_API_REG_NDEF_HDLR *p_init_handler,
                              uint8_t tnf, uint8_t *p_type_name,
                              uint8_t type_name_len, uint8_t *p_payload,
...
void nfa_dm_ndef_handle_message(tNFA_STATUS status, uint8_t *p_msg_buf,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_ndef_handle_message status=%i, msgbuf=%p, len=%i",
