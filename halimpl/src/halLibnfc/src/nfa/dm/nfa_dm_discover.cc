...
static uint8_t
nfa_dm_get_rf_discover_config(tNFA_DM_DISC_TECH_PROTO_MASK dm_disc_mask,
                              tNFC_DISCOVER_PARAMS disc_params[],
                              uint8_t max_params);
__attribute__((unused)) static tNFA_STATUS
nfa_dm_set_rf_listen_mode_config(tNFA_DM_DISC_TECH_PROTO_MASK tech_proto_mask);
static void
nfa_dm_set_rf_listen_mode_raw_config(tNFA_DM_DISC_TECH_PROTO_MASK *p_disc_mask);
static tNFA_DM_DISC_TECH_PROTO_MASK
nfa_dm_disc_get_disc_mask(tNFC_RF_TECH_N_MODE tech_n_mode,
...
nfa_dm_get_rf_discover_config(tNFA_DM_DISC_TECH_PROTO_MASK dm_disc_mask,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s : dm_disc_mask = 0x%08X", __func__, dm_disc_mask);
...
static tNFA_STATUS nfa_dm_set_listen_mode_set_configs(
  ...
  if (NfcConfig::hasKey(NAME_HOST_LISTEN_TECH_MASK)) {
    hostListenMask = NfcConfig::getUnsigned(NAME_HOST_LISTEN_TECH_MASK);
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "%s : HOST_LISTEN_TECH_MASK = 0x0%lu;", __func__, hostListenMask);
  }
  ...
  if (tech_proto_mask & NFA_DM_DISC_MASK_LA_NFC_DEP) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s : gLastSak = 0x%X;", __func__, gLastSak);
    if (gLastSak)
      SAK |= gLastSak;
    SAK |= NCI_PARAM_SEL_INFO_NFCDEP;
    gLastSak &= 0x00;
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s : LA_SEL_INFO = 0x%X;", __func__, SAK);
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s : LB_SENSB_INFO = 0x%X;", __func__, SENSB_RES);
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s : LF_PROTOCOL_TYPE = 0x%X;", __func__, LF_PROTOTYPE);
...
nfa_dm_set_rf_listen_mode_config(tNFA_DM_DISC_TECH_PROTO_MASK tech_proto_mask) {
  ...
  */
  ...
  if (tech_proto_mask & NFA_DM_DISC_MASK_LA_T1T) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: sens_info=0x0%x;", __func__, sens_info);
#endif
  }
#if (NXP_EXTNS == TRUE)
  /*
...
static void nfa_dm_set_total_duration(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_set_total_duration ()");
...
static void nfa_dm_set_rf_listen_mode_raw_config(
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_set_rf_listen_mode_raw_config ()");
  ...
  *p_disc_mask = disc_mask;
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_dm_set_rf_listen_mode_raw_config () disc_mask = 0x%x", disc_mask);
}
...
static tNFC_STATUS nfa_dm_disc_force_to_idle(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_disc_force_to_idle() disc_flags = 0x%x",
  ...
  if (!w4_flags) {
    ...
    if (!nfa_dm_cb.disc_cb.tle.in_use) {
      ...
      if (nfc_cb.num_disc_maps == 1) {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("Reading NAME_NFA_DM_DISC_NTF_TIMEOUT val   "
...
void nfa_dm_start_rf_discover(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_start_rf_discover (): enter");
  ...
  if ((!(nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_ENABLED)) &&
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_dm_start_rf_discover (): return");
    return;
  }
  ...
  if (nfa_dm_cb.disc_cb.excl_disc_entry.in_use) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "dm_disc_mask calculated finally for discovery: 0x%X", dm_disc_mask);
...
#if (NXP_EXTNS == TRUE)
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("dm_disc_mask before masking = 0x%x", dm_disc_mask);
  if (((hostListenMask & 0x1) == 0x0) && ((uiccListenMask & 0x1) == 0x0) &&
      ((eseListenMask & 0x1) == 0x0)) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("dm_disc_mask after masking = 0x%x", dm_disc_mask);
#endif
...
static tNFA_STATUS nfa_dm_disc_notify_activation(tNFC_DISCOVER *p_data) {
   ...
   * return (NFA_STATUS_OK)
   */
  if (p_data->activate.intf_param.type ==
...
static void nfa_dm_disc_kovio_timeout_cback(__attribute__((unused))
                                            TIMER_LIST_ENT *p_tle) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_disc_kovio_timeout_cback()");
...
tNFC_STATUS nfa_dm_disc_start_kovio_presence_check(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_disc_start_kovio_presence_check ()");
...
static void nfa_dm_disc_report_kovio_presence_check(tNFC_STATUS status) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_disc_report_kovio_presence_check ()");
...
static void nfa_dm_disc_sm_idle(tNFA_DM_RF_DISC_SM_EVENT event,
  ...
  default:
    LOG(ERROR) << StringPrintf(
        "nfa_dm_disc_sm_idle (): Unexpected discovery event");
    break;
  }
}
...
static void nfa_dm_disc_sm_w4_all_discoveries(tNFA_DM_RF_DISC_SM_EVENT event,
  ...
  case NFA_DM_RF_DISCOVER_NTF:
    ...
    if (p_data->nfc_discover.result.protocol == NCI_PROTOCOL_UNKNOWN) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("Unknown protocol - Restart Discovery");
...
#if (NXP_EXTNS == TRUE)
    LOG(ERROR) << StringPrintf(
        "nfa_dm_disc_sm_w4_host_select (): Restarted discovery");
    NFC_Deactivate(NFA_DEACTIVATE_TYPE_IDLE);
#endif
    break;
  }
...
tNFA_HANDLE nfa_dm_add_rf_discover(tNFA_DM_DISC_TECH_PROTO_MASK disc_mask,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_add_rf_discover ():host_id=0x%X disc_mask=0x%x",
                      host_id, disc_mask);
...
void nfa_dm_start_excl_discovery(tNFA_TECHNOLOGY_MASK poll_tech_mask,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_start_excl_discovery ()");
...
void nfa_dm_stop_excl_discovery(void) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_stop_excl_discovery ()");
...
void nfa_dm_delete_rf_discover(tNFA_HANDLE handle) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_delete_rf_discover () handle=0x%x", handle);
...
tNFA_STATUS nfa_dm_rf_deactivate(tNFA_DEACTIVATE_TYPE deactivate_type) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_dm_rf_deactivate () deactivate_type:0x%X", deactivate_type);
  ...
  if (nfa_dm_cb.disc_cb.disc_state == NFA_DM_RFST_IDLE) {
    ...
    if (deactivate_type == NFA_DEACTIVATE_TYPE_DISCOVERY) {
      if (nfa_dm_cb.disc_cb.kovio_tle.in_use) {
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "nfa_dm_rf_deactivate (): already in discovery state");
        return NFA_STATUS_FAILED;
      }
...
#if (NXP_EXTNS == TRUE)
    if (type == nfcFL.nfcMwFL._NCI_INTERFACE_UICC_DIRECT ||
        type == nfcFL.nfcMwFL._NCI_INTERFACE_ESE_DIRECT) {
      DLOG_IF(INFO, nfc_debug_enabled)
...
void NFA_SetEmvCoState(bool flag) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_SetEmvCoState = 0x%x", is_emvco_active);
}
#endif
/*******************************************************************************
**
