...
tNFA_STATUS NFA_SetConfig(tNFA_PMID param_id, uint8_t length, uint8_t *p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_SetConfig (): param_id:0x%X", param_id);
...
tNFA_STATUS NFA_GetConfig(uint8_t num_ids, tNFA_PMID *p_param_ids) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_GetConfig (): num_ids: %i", num_ids);
...
tNFA_STATUS NFA_RequestExclusiveRfControl(tNFA_TECHNOLOGY_MASK poll_mask,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_RequestExclusiveRfControl () poll_mask=0x%x", poll_mask);

  if (!p_conn_cback) {
    LOG(ERROR) << StringPrintf(
        "NFA_RequestExclusiveRfControl (): error null callback");
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_EnablePolling(tNFA_TECHNOLOGY_MASK poll_mask) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EnablePolling () 0x%X", poll_mask);

  p_msg =
      (tNFA_DM_API_ENABLE_POLL *)GKI_getbuf(sizeof(tNFA_DM_API_ENABLE_POLL));
  if (p_msg != NULL) {
...
tNFA_STATUS NFA_DisablePassiveListening(void) {
  ...
  if (!nfcFL.eseFL._NFCC_ESE_UICC_CONCURRENT_ACCESS_PROTECTION) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf(" NFCC_ESE_UICC_CONCURRENT_ACCESS_PROTECTION"
...
void NFA_SetPreferredUiccId(uint8_t uicc_id) {
  ...
  if (!nfcFL.nfccFL._NFC_NXP_STAT_DUAL_UICC_WO_EXT_SWITCH) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFC_NXP_STAT_DUAL_UICC_WO_EXT_SWITCH"
                        " feature is not available!!");
    return;
  }
...
tNFA_STATUS NFA_SetP2pListenTech(tNFA_TECHNOLOGY_MASK tech_mask) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pSetListenTech (): tech_mask:0x%X", tech_mask);
...
tNFA_STATUS NFA_Select(uint8_t rf_disc_id, tNFA_NFC_PROTOCOL protocol,
  ...
  if (((rf_interface == NFA_INTERFACE_ISO_DEP) &&
    ...
    LOG(ERROR) << StringPrintf(
        "NFA_Select (): RF interface is not matched protocol");
...
extern tNFA_STATUS NFA_Deactivate(bool sleep_mode) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_Deactivate (): sleep_mode:%i", sleep_mode);
...
tNFA_STATUS NFA_SendRawFrame(uint8_t *p_raw_data, uint16_t data_len,
  ...
  if (data_len > p_cb->buff_size) {
    if (nfa_dm_get_extended_cmd_buf((tNFC_EXT_HDR **)&p_msg,
                                    NFA_DM_API_RAW_FRAME_EVT, p_raw_data,
                                    data_len)) {
      LOG(ERROR) << StringPrintf("NFA_SendRawFrame () memory allocation for "
...
tNFA_STATUS NFA_RegisterNDefTypeHandler(bool handle_whole_message, tNFA_TNF tnf,
  ...
  if (!p_ndef_cback) {
    LOG(ERROR) << StringPrintf(
        "NFA_RegisterNDefTypeHandler (): error - null callback");
    return (NFA_STATUS_INVALID_PARAM);
  }
...
extern tNFA_STATUS NFA_RegisterNDefUriHandler(bool handle_whole_message,
  ...
  if (!p_ndef_cback) {
    LOG(ERROR) << StringPrintf(
        "NFA_RegisterNDefUriHandler (): error - null callback");
    return (NFA_STATUS_INVALID_PARAM);
  }
...
extern tNFA_STATUS NFA_DeregisterNDefTypeHandler(tNFA_HANDLE ndef_type_handle) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_DeregisterNDefHandler (): handle 0x%08x", ndef_type_handle);
...
tNFA_STATUS NFA_PowerOffSleepMode(bool start_stop) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_PowerOffSleepState () start_stop=%d", start_stop);
...
void NFA_SetReaderMode(bool ReaderModeFlag, uint32_t Technologies) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_SetReaderMode =0x%x", ReaderModeFlag);
...
tNFA_MW_VERSION NFA_GetMwVersion() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "mwVer:Major=0x%x,Minor=0x%x", mwVer.major_version, mwVer.minor_version);
  return mwVer;
}
...
**
*******************************************************************************/
void NFA_SetLowRamDevice(bool mode) { hal_Initcntxt.isLowRam = mode; }
...
void NFA_EE_HCI_Control(bool flagEnable) {
  ...
  if (!flagEnable) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_EE_HCI_Control (); Disable system");
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_EE_HCI_Control (); Enable system");
    for (i = 0; i < 2; i++) {
      if (nfa_sys_cb.is_reg[id[i]]) {
...
**
*******************************************************************************/
tNFA_STATUS NFA_Send_Core_Reset() { return nfc_hal_nfcc_reset(); }
...
**
*******************************************************************************/
void NFA_Send_Core_Init(uint8_t **p) { nfc_hal_nfcc_init(p); }
