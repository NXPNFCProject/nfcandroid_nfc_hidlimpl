...
tNFA_STATUS nfa_dm_check_set_config(uint8_t tlv_list_len, uint8_t *p_tlv_list,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_check_set_config () tlv_len=%d", tlv_list_len);
...
#if (NXP_EXTNS == TRUE)
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_dm_check_set_config () updated_len=%d", updated_len);
    if (!updated_len) {
...
** Returns          NFA_STATUS_FAILED, If memory allocation is not successfull.
                    NFA_STATUS_OK, If memory allocation and pointer
...
tNFA_STATUS nfa_dm_get_extended_cmd_buf(tNFC_EXT_HDR **p_msg, uint32_t event,
  ...
  if (p_data) {
    ...
    } else if (event == NFA_P2P_API_SEND_UI_EVT ||
               event == NFA_P2P_API_SEND_DATA_EVT) {
