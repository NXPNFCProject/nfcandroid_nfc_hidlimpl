...
static void nfa_rw_store_ndef_rx_buf(tRW_DATA *p_rw_data) {
  ...
  if (NULL == p_rw_data) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_rw_store_ndef_rx_buf;  p_rw_data is NULL");
    return;
  }
...
*******************************************************************************/
static void
nfa_rw_check_start_presence_check_timer(uint16_t presence_check_start_delay) {
...
static void nfa_rw_handle_t3t_evt(tRW_EVENT event, tRW_DATA *p_rw_data) {
  ...
  default:
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_rw_handle_t3t_evt(); Unhandled RW event 0x%X", event);
    break;
  }
}
...
static void nfa_rw_handle_t4t_evt(tRW_EVENT event, tRW_DATA *p_rw_data) {
  ...
  default:
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_rw_handle_t4t_evt(); Unhandled RW event 0x%X", event);
    break;
  }
}
...
static void nfa_rw_handle_i93_evt(tRW_EVENT event, tRW_DATA *p_rw_data) {
  ...
  default:
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_rw_handle_i93_evt(); Unhandled RW event 0x%X", event);
    break;
  }
}
#if (NXP_EXTNS == TRUE)
...
static void nfa_rw_cback(tRW_EVENT event, tRW_DATA *p_rw_data) {
  ...
  if (event < RW_T1T_MAX_EVT) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_rw_cback: unhandled event=0x%02x", event);
  }
}
...
static bool nfa_rw_i93_command(tNFA_RW_MSG *p_data) {
  ...
  case NFA_RW_OP_I93_READ_SINGLE_BLOCK:
    ...
    status =
        RW_I93ReadSingleBlock(p_data->op_req.params.i93_cmd.first_block_number);
    break;

  case NFA_RW_OP_I93_WRITE_SINGLE_BLOCK:
    i93_command = I93_CMD_WRITE_SINGLE_BLOCK;
    status =
        RW_I93WriteSingleBlock(p_data->op_req.params.i93_cmd.first_block_number,
...
static void nfa_rw_raw_mode_data_cback(__attribute__((unused)) uint8_t conn_id,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_rw_raw_mode_data_cback(): event = 0x%X", event);
...
bool nfa_rw_handle_op_req(tNFA_RW_MSG *p_data) {
  ...
  default:
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_rw_handle_api: unhandled operation: %i", p_data->op_req.op);
    break;
  }
...
void nfa_rw_set_cback(tNFC_DISCOVER *p_data) {
  ...
  if ((p_data != NULL) &&
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_rw_set_cback: nfa_rw_raw_mode_data_cback");
    /* Set data callback (pass all incoming data to upper layer using
     * NFA_DATA_EVT) */
    NFC_SetStaticRfCback(nfa_rw_raw_mode_data_cback);
  }
