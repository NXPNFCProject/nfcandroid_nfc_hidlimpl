...
*******************************************************************************/
tNFA_STATUS
NFA_CeConfigureLocalTag(tNFA_PROTOCOL_MASK protocol_mask, uint8_t *p_ndef_data,
                        uint16_t ndef_cur_size, uint16_t ndef_max_size,
                        bool read_only, uint8_t uid_len, uint8_t *p_uid)
...
tNFA_STATUS NFA_CeConfigureEseListenTech(tNFA_HANDLE ee_handle,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_CeConfigureEseListenTech () ee_handle = 0x%x", ee_handle);
...
tNFA_STATUS NFA_CeDeregisterFelicaSystemCodeOnDH(tNFA_HANDLE handle) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_CeDeregisterFelicaSystemCodeOnDH (): handle:0x%X", handle);
...
tNFA_STATUS NFA_CeDeregisterAidOnDH(tNFA_HANDLE handle) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_CeDeregisterAidOnDH (): handle:0x%X", handle);
...
tNFA_STATUS NFA_CeSetIsoDepListenTech(tNFA_TECHNOLOGY_MASK tech_mask) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_CeSetIsoDepListenTech (): 0x%x", tech_mask);
