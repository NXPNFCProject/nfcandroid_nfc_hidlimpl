...
void nfa_ce_handle_t3t_evt(tCE_EVENT event, tCE_DATA *p_ce_data) {
  ...
  default:
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_ce_handle_t3t_evt unhandled event=0x%02x", event);
    break;
  }
}
...
void nfa_ce_handle_t4t_evt(tCE_EVENT event, tCE_DATA *p_ce_data) {
  ...
  default:
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_ce_handle_t4t_evt unhandled event=0x%02x", event);
    break;
  }
}
...
void nfa_ce_discovery_cback(tNFA_DM_RF_DISC_EVT event, tNFC_DISCOVER *p_data) {
  tNFA_CE_MSG ce_msg;
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ce_discovery_cback(): event:0x%02X", event);
...
tNFA_STATUS nfa_ce_start_listening(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ce_start_listening(): enter");
  /*************************************************************************/
...
#if (NXP_EXTNS == TRUE)
          DLOG_IF(INFO, nfc_debug_enabled)
              << StringPrintf("UICC/ESE[0x%x] is not activated",
                              p_cb->listen_info[listen_info_idx].ee_handle);
#else
          DLOG_IF(INFO, nfc_debug_enabled)
              << StringPrintf("UICC[0x%x] is not activated",
                              p_cb->listen_info[listen_info_idx].ee_handle);
#endif
        }
      }
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ce_start_listening(): exit");
...
tNFC_STATUS nfa_ce_set_content(void) {
  ...
  if (status != NFA_STATUS_OK) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("Unable to set contents (error %02x)", status);
  }
...
bool nfa_ce_activate_ntf(tNFA_CE_MSG *p_ce_msg) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ce_activate_ntf () protocol=%d",
...
#if (NXP_EXTNS == TRUE)
  if (p_cb->activation_params.intf_param.type ==
          nfcFL.nfcMwFL._NCI_INTERFACE_UICC_DIRECT ||
      p_cb->activation_params.intf_param.type ==
...
bool nfa_ce_deactivate_ntf(tNFA_CE_MSG *p_ce_msg) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ce_deactivate_ntf () deact_type=%d", deact_type);
...
bool nfa_ce_api_reg_listen(tNFA_CE_MSG *p_ce_msg) {
  ...
  for (i = 1; i < NFA_CE_LISTEN_INFO_MAX; i++) {
    if ((p_ce_msg->reg_listen.listen_type == NFA_CE_REG_TYPE_UICC) &&
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("UICC (0x%x) listening already specified",
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("ESE (0x%x) listening already specified",
                          p_ce_msg->reg_listen.ee_handle);
      conn_evt.status = NFA_STATUS_FAILED;
  ...
  if (listen_info_idx == NFA_CE_LISTEN_INFO_IDX_INVALID) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "Maximum listen callbacks exceeded (%i)", NFA_CE_LISTEN_INFO_MAX);
