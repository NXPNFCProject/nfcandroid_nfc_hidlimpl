...
tNFA_STATUS NFA_P2pRegisterClient(tNFA_P2P_LINK_TYPE link_type,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pRegisterClient (): link_type:0x%x", link_type);
...
tNFA_STATUS NFA_P2pDeregister(tNFA_HANDLE handle) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pDeregister (): handle:0x%02X", handle);
...
tNFA_STATUS NFA_P2pAcceptConn(tNFA_HANDLE handle, uint16_t miu, uint8_t rw) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_P2pAcceptConn (): handle:0x%02X, MIU:%d, RW:%d", handle, miu, rw);

  xx = handle & NFA_HANDLE_MASK;

  if (!(xx & NFA_P2P_HANDLE_FLAG_CONN)) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pAcceptConn (): Connection Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  } else {
  ...
  if ((miu < LLCP_DEFAULT_MIU) || (nfa_p2p_cb.local_link_miu < miu)) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pAcceptConn (): MIU(%d) must be between %d and %d", miu,
...
tNFA_STATUS NFA_P2pRejectConn(tNFA_HANDLE handle) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pRejectConn (): handle:0x%02X", handle);

  xx = handle & NFA_HANDLE_MASK;

  if (!(xx & NFA_P2P_HANDLE_FLAG_CONN)) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pRejectConn (): Connection Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  } else {
    xx &= ~NFA_P2P_HANDLE_FLAG_CONN;
  }
...
tNFA_STATUS NFA_P2pDisconnect(tNFA_HANDLE handle, bool flush) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_P2pDisconnect (): handle:0x%02X, flush=%d", handle, flush);
  ...
  if (xx & NFA_P2P_HANDLE_FLAG_CONN) {
    ...
    if ((xx >= LLCP_MAX_DATA_LINK) || (nfa_p2p_cb.conn_cb[xx].flags == 0)) {
      LOG(ERROR) << StringPrintf(
          "NFA_P2pDisconnect (): Connection Handle is not valid");
      return (NFA_STATUS_BAD_HANDLE);
    }
  } else {
...
tNFA_STATUS NFA_P2pConnectByName(tNFA_HANDLE client_handle,
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pConnectByName (): Client Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  }
...
tNFA_STATUS NFA_P2pConnectBySap(tNFA_HANDLE client_handle, uint8_t dsap,
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pConnectBySap (): Client Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  }
...
tNFA_STATUS NFA_P2pSendUI(tNFA_HANDLE handle, uint8_t dsap, uint16_t length,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pSendUI (): handle:0x%X, DSAP:0x%02X, length:%d",
                      handle, dsap, length);
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    LOG(ERROR) << StringPrintf("NFA_P2pSendUI (): Handle (0x%X) is not valid",
                               handle);
    ...
    if ((length + LLCP_PDU_HEADER_SIZE) > p_cb->buff_size) {
      if (nfa_dm_get_extended_cmd_buf((tNFC_EXT_HDR **)&p_msg->p_msg,
                                      NFA_P2P_API_SEND_UI_EVT, p_data,
                                      length)) {
        LOG(ERROR) << StringPrintf("NFA_P2pSendUI () memory allocation for "
                                   "extended length is un-successfull");
        return NFA_STATUS_FAILED;
      } else {
        /*Do nothing*/
      }
...
tNFA_STATUS NFA_P2pReadUI(tNFA_HANDLE handle, uint32_t max_data_len,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pReadUI (): handle:0x%X", handle);
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    LOG(ERROR) << StringPrintf("NFA_P2pReadUI (): Handle (0x%X) is not valid",
                               handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
  } else {
...
tNFA_STATUS NFA_P2pFlushUI(tNFA_HANDLE handle, uint32_t *p_length) {
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    LOG(ERROR) << StringPrintf("NFA_P2pFlushUI (): Handle (0x%X) is not valid",
                               handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
    *p_length = 0;
  } else {
...
tNFA_STATUS NFA_P2pSendData(tNFA_HANDLE handle, uint16_t length,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_P2pSendData (): handle:0x%X, length:%d", handle, length);
  ...
  if ((!(handle & NFA_P2P_HANDLE_FLAG_CONN)) || (xx >= LLCP_MAX_DATA_LINK) ||
    ...
    LOG(ERROR) << StringPrintf("NFA_P2pSendData (): Handle(0x%X) is not valid",
                               handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
    ...
    if ((length + LLCP_PDU_HEADER_SIZE) > p_cb->buff_size) {
      if (nfa_dm_get_extended_cmd_buf((tNFC_EXT_HDR **)&p_msg->p_msg,
                                      NFA_P2P_API_SEND_DATA_EVT, p_data,
                                      length)) {
        LOG(ERROR) << StringPrintf("NFA_P2pSendData () memory allocation for "
                                   "extended length is un-successfull");
        return NFA_STATUS_FAILED;
      } else {
        /*Do nothing*/
...
tNFA_STATUS NFA_P2pReadData(tNFA_HANDLE handle, uint32_t max_data_len,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pReadData (): handle:0x%X", handle);
  ...
  if ((!(handle & NFA_P2P_HANDLE_FLAG_CONN)) || (xx >= LLCP_MAX_DATA_LINK) ||
      (nfa_p2p_cb.conn_cb[xx].flags == 0)) {
    LOG(ERROR) << StringPrintf("NFA_P2pReadData (): Handle(0x%X) is not valid",
                               handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
  } else {
...
tNFA_STATUS NFA_P2pFlushData(tNFA_HANDLE handle, uint32_t *p_length) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pFlushData (): handle:0x%X", handle);
  ...
  if ((!(handle & NFA_P2P_HANDLE_FLAG_CONN)) || (xx >= LLCP_MAX_DATA_LINK) ||
      (nfa_p2p_cb.conn_cb[xx].flags == 0)) {
    LOG(ERROR) << StringPrintf("NFA_P2pFlushData (): Handle(0x%X) is not valid",
                               handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
  } else {
...
tNFA_STATUS NFA_P2pSetLocalBusy(tNFA_HANDLE conn_handle, bool is_busy) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pSetLocalBusy (): conn_handle:0x%02X, is_busy:%d",
                      conn_handle, is_busy);
  ...
  if (!(xx & NFA_P2P_HANDLE_FLAG_CONN)) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pSetLocalBusy (): Connection Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  } else {
    xx &= ~NFA_P2P_HANDLE_FLAG_CONN;
  }
...
tNFA_STATUS NFA_P2pGetLinkInfo(tNFA_HANDLE handle) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_P2pGetLinkInfo (): handle:0x%x", handle);

  if (nfa_p2p_cb.llcp_state != NFA_P2P_LLCP_STATE_ACTIVATED) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pGetLinkInfo (): LLCP link is not activated");
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_P2pGetRemoteSap(tNFA_HANDLE handle, char *p_service_name) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_P2pGetRemoteSap(): handle:0x%x, SN:<%s>", handle, p_service_name);

  if (nfa_p2p_cb.llcp_state != NFA_P2P_LLCP_STATE_ACTIVATED) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pGetRemoteSap(): LLCP link is not activated");
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_P2pSetLLCPConfig(uint16_t link_miu, uint8_t opt, uint8_t wt,
  ...
  if (nfa_p2p_cb.llcp_state == NFA_P2P_LLCP_STATE_ACTIVATED) {
    LOG(ERROR) << StringPrintf(
        "NFA_P2pSetLLCPConfig (): LLCP link is activated");
    return (NFA_STATUS_FAILED);
  }
