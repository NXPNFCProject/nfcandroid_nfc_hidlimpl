...
tNFA_STATUS NFA_HciRegister(char *p_app_name, tNFA_HCI_CBACK *p_cback,
  ...
  if (p_app_name == NULL) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf(" %s: NFAInvalid Application name", __func__);
    return (NFA_STATUS_FAILED);
  }

  if (p_cback == NULL) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "%s: Application should provide callback function to register!",
        __func__);
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_HciDeregister(char *p_app_name) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("N%s: Application Name: %s", __func__, p_app_name);
  app_name_len = (uint8_t)strlen(p_app_name);
  ...
  if (xx == NFA_HCI_MAX_APP_CB) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "%s: Application Name: %s  NOT FOUND", __func__, p_app_name);
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_HciGetHostList(tNFA_HANDLE hci_handle) {
  ...
  if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("N%s: Invalid hci_handle:0x%04x", __func__, hci_handle);
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_HciOpenPipe(tNFA_HANDLE hci_handle, uint8_t pipe) {
  ...
  if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciOpenPipe (): Invalid hci_handle:0x%04x", hci_handle);
    return (NFA_STATUS_FAILED);
  }
  ...
  if ((pipe < NFA_HCI_FIRST_DYNAMIC_PIPE) ||
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_HciOpenPipe (): Invalid Pipe:0x%02x", pipe);
    return (NFA_STATUS_FAILED);
  }

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_HciOpenPipe (): hci_handle:0x%04x, pipe:0x%02X", hci_handle, pipe);
...
tNFA_STATUS NFA_HciGetRegistry(tNFA_HANDLE hci_handle, uint8_t pipe,
  ...
  if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciGetRegistry (): Invalid hci_handle:0x%04x", hci_handle);
    return (NFA_STATUS_FAILED);
  }

  if (pipe < NFA_HCI_FIRST_DYNAMIC_PIPE) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_HciGetRegistry (): Invalid Pipe:0x%02x", pipe);
    return (NFA_STATUS_FAILED);
  }

  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_HciGetRegistry (): hci_handle:0x%04x  Pipe: 0x%02x",
                      hci_handle, pipe);
...
extern tNFA_STATUS NFA_HciSetRegistry(tNFA_HANDLE hci_handle, uint8_t pipe,
  ...
  if (pipe < NFA_HCI_FIRST_DYNAMIC_PIPE) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_HciSetRegistry (): Invalid Pipe:0x%02x", pipe);
    return (NFA_STATUS_FAILED);
  }
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_HciSetRegistry (): hci_handle:0x%04x  Pipe: 0x%02x",
                      hci_handle, pipe);
...
tNFA_STATUS NFA_HciSendCommand(tNFA_HANDLE hci_handle, uint8_t pipe,
  ...
  if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciSendCommand (): Invalid hci_handle:0x%04x", hci_handle);
    return (NFA_STATUS_FAILED);
  }

  if (pipe < NFA_HCI_FIRST_DYNAMIC_PIPE) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_HciSendCommand (): Invalid Pipe:0x%02x", pipe);
    return (NFA_STATUS_FAILED);
  }

  if ((cmd_size && (p_data == NULL)) || (cmd_size > NFA_MAX_HCI_CMD_LEN)) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciSendCommand (): Invalid cmd size:0x%02x", cmd_size);
    return (NFA_STATUS_FAILED);
  }
...
#endif
{
  ...
  if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciSendEvent (): Invalid hci_handle:0x%04x", hci_handle);
    return (NFA_STATUS_FAILED);
  }

  if (pipe < NFA_HCI_FIRST_DYNAMIC_PIPE) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_HciSendEvent (): Invalid Pipe:0x%02x", pipe);
    return (NFA_STATUS_FAILED);
  }

  if (evt_size && (p_data == NULL)) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciSendEvent (): Invalid Event size:0x%02x", evt_size);
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_HciClosePipe(tNFA_HANDLE hci_handle, uint8_t pipe) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_HciClosePipe (): hci_handle:0x%04x, pipe:0x%02X", hci_handle, pipe);

  if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciClosePipe (): Invalid hci_handle:0x%04x", hci_handle);
    return (NFA_STATUS_FAILED);
  }
  ...
  if ((pipe < NFA_HCI_FIRST_DYNAMIC_PIPE) ||
      (pipe > NFA_HCI_LAST_DYNAMIC_PIPE)) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_HciClosePipe (): Invalid Pipe:0x%02x", pipe);
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_HciDeletePipe(tNFA_HANDLE hci_handle, uint8_t pipe) {
  ...
  if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciDeletePipe (): Invalid hci_handle:0x%04x", hci_handle);
    return (NFA_STATUS_FAILED);
  }
  ...
  if ((pipe < NFA_HCI_FIRST_DYNAMIC_PIPE) ||
      (pipe > NFA_HCI_LAST_DYNAMIC_PIPE)) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_HciDeletePipe (): Invalid Pipe:0x%02x", pipe);
    return (NFA_STATUS_FAILED);
  }

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_HciDeletePipe (): hci_handle:0x%04x, pipe:0x%02X", hci_handle, pipe);
...
tNFA_STATUS NFA_HciAddStaticPipe(tNFA_HANDLE hci_handle, uint8_t host,
  ...
  if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_HciAddStaticPipe (): Invalid hci_handle:0x%04x", hci_handle);
    return (NFA_STATUS_FAILED);
  }
...
#endif
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_HciAddStaticPipe (): hci_handle:0x%04x, pipe:0x%02X",
                      hci_handle, pipe);
...
bool NFA_MW_Fwdnlwd_Recovery(bool mw_fwdnld_recovery) {
  if (!nfcFL.nfccFL._NFCC_MW_RCVRY_BLK_FW_DNLD) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_MW_Fwdnlwd_Recovery "
...
void NFA_HciW4eSETransaction_Complete(tNFA_HCI_TRANSCV_STATE type) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_HciW4eSETransaction_Complete; type=%u", type);
  uint8_t retry_cnt = 0;
...
tNFA_STATUS NFA_HciConfigureNfceeETSI12() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_HciConfigureNfceeETSI12 (): Entry");
  ...
  if ((nfa_hci_cb.hci_state != NFA_HCI_STATE_DISABLED) &&
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_HciConfigureNfceeETSI12 (): Entry HCI state - %d",
...
bool NFA_IsPipeStatusNotCorrect() {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_IsPipeStatusNotCorrect (): pipe status not correct: %d",
      nfa_hci_cb.IsApduPipeStatusNotCorrect);
  bool status;
