...
nfa_hci_api_get_host_id(tNFA_HCI_EVENT_DATA *p_evt_data);
static __attribute__((unused)) void
nfa_hci_api_get_host_type(tNFA_HCI_EVENT_DATA *p_evt_data);
...
nfa_hci_api_GetpipeId(uint8_t nfceeId, uint8_t gateId, uint8_t *pipeId);
static void nfa_hci_poll_session_id_cb(uint8_t event, uint16_t param_len,
                                       uint8_t *p_param);
static __attribute__((unused)) void
nfa_hci_read_num_nfcee_config_cb(uint8_t event, uint16_t param_len,
                                 uint8_t *p_param);
static tNFA_STATUS nfa_hci_poll_session_id(uint8_t host_type);
static void nfa_hci_get_pipe_state_cb(uint8_t event, uint16_t param_len,
                                      uint8_t *p_param);
...
void nfa_hci_check_pending_api_requests(void) {
  ...
  case NFA_HCI_API_CONFIGURE_EVT:
    if (nfa_hci_api_config_nfcee(p_evt_data->config_info.config_nfcee_event) ==
        false)
      b_free = false;
    break;
#endif
  ...
  for (;;) {
    ...
    default:
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "nfa_hci_check_api_requests ()  Unknown event: 0x%04x", p_msg->event);
      break;
    }
  ...
  if (xx != NFA_HCI_MAX_APP_CB) {
    ...
    if (xx == NFA_HCI_MAX_APP_CB) {
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("nfa_hci_api_register (%s)  NO ENTRIES", p_app_name);
...
#else
      response = nfa_hciu_add_pipe_to_static_gate(dest_gate, pipe, source_host,
                                                  source_gate);
#endif
    else {
  ...
  if ((nfa_hci_cb.hci_state == NFA_HCI_STATE_STARTUP) ||
    ...
    case NFA_HCI_ANY_SET_PARAMETER:
      ...
      if (nfa_hci_cb.param_in_use == NFA_HCI_WHITELIST_INDEX) {
        ...
        nfa_hciu_send_set_param_cmd(NFA_HCI_ADMIN_PIPE, NFA_HCI_HOST_TYPE_INDEX,
...
void nfa_hci_handle_dyn_pipe_pkt(uint8_t pipe_id, uint8_t *p_data,
  ...
  if (p_pipe == NULL) {
    /* Invalid pipe ID */
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_hci_handle_dyn_pipe_pkt - Unknown pipe %d", pipe_id);
...
tNFA_STATUS nfa_hci_api_get_host_type_list() {
  ...
  tNFA_HCI_EVT_DATA evt_data;
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_api_get_host_type_list - enter!!");
...
tNFA_STATUS nfa_hci_api_config_nfcee(uint8_t hostId) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_hci_api_config_nfcee - No NFCEE Config is needed!!!");
...
static tNFA_STATUS nfa_hci_get_num_nfcee_configured() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_read_num_nfcee_config_cb  nfa_ee_max_ee_cfg%x",
                      nfa_ee_max_ee_cfg);
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_read_num_nfcee_config_cb nfa_hci_cb.num_nfcee%x",
...
static tNFA_STATUS nfa_hci_poll_all_nfcee_session_id() {
  ...
  for (xx = 0; xx < nfa_hci_cb.num_nfcee; xx++) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_hci_pollsession_id   -%x", nfa_hci_cb.hci_ee_info[xx].ee_handle);
    if ((((nfa_hci_cb.hci_ee_info[xx].ee_interface[0] !=
           NCI_NFCEE_INTERFACE_HCI_ACCESS) &&
          nfa_hci_cb.hci_ee_info[xx].ee_status == NFA_EE_STATUS_ACTIVE)) &&
        (nfa_hci_cb.hci_ee_info[xx].ee_handle != 0x410)) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_poll_all_nfcee_session_id - exit!!");
  return status;
}
...
static tNFA_STATUS nfa_hci_poll_session_id(uint8_t host_type) {
  ...
  if (NFA_HCI_HOST_ID_UICC0 == host_type ||
      NFA_HCI_HOST_ID_UICC0_NCI2 == host_type) {
  ...
  if (status == NFA_STATUS_OK && ((p - p_data) > 0x00)) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_hci_clear_all_pipe_ntf session id read");
...
**
*******************************************************************************/
static void nfa_hci_poll_session_id_cb(__attribute__((unused)) uint8_t event,
  ...
  if (!memcmp((uint8_t *)default_session, p_param + SESSION_ID_INDEX,
              NFA_HCI_SESSION_ID_LEN)) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_hci_poll_session_id_cb invalid session id");
    if (nfa_hci_cb.nfcee_cfg.session_id_retry <=
        nfa_hci_cb.max_hci_session_id_read_count) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_hci_poll_session_id_cb valid session id");
    nfa_hci_cb.nfcee_cfg.session_id_retry =
...
void nfa_hci_handle_nfcee_config_evt(uint16_t event) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_handle_nfcee_config_evt enter %x", event);
  switch (event) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_handle_nfcee_config_evt exit");
}
...
static void nfa_hci_api_getnoofhosts(uint8_t *p_data, uint8_t data_len) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_hci_api_getnoofhosts :-no of hosts in HCI Network-%d", noofhosts);
  for (count = 0; count < data_len; count++) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_hci_api_getnoofhosts :data[%d] - %d\n", count, p_data[count]);
  }
  for (count = 0; count < noofhosts; count++) {
    ...
    if ((p_data[host_id] & p_data[host_id + 1]) != 0xFF) {
      if ((p_data[host_id] == NFA_HCI_HOST_ID_UICC0) &&
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("nfa_hci_api_getnoofhosts :- UICC !!!!");
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("nfa_hci_api_getnoofhosts :- eSE !!!!");
...
static bool nfa_hci_api_checkforAPDUGate(uint8_t *p_data, uint8_t data_len) {
  ...
  for (count = 0; count < data_len; count++) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_hci_api_checkforAPDUGate -Gate id -- %d !!!", p_data[count]);
    if (p_data[count] == NFA_HCI_ETSI12_APDU_GATE) {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "nfa_hci_api_checkforAPDUGate -count -- %d !!!", count);
      status = true;
      break;
    }
  }
...
static void nfa_hci_handle_Nfcee_admpipe_rsp(uint8_t *p_data,
  ...
  if (nfa_hci_cb.inst != NFA_HCI_ANY_OK) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_hci_handle_Nfcee_admpipe_rsp - Opening pipe!!!");
...
static void nfa_hci_handle_Nfcee_dynpipe_rsp(uint8_t pipeId, uint8_t *p_data,
  ...
  if (nfa_hci_cb.type == NFA_HCI_RESPONSE_TYPE) {
    nfa_sys_stop_timer(&nfa_hci_cb.timer);
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_hci_handle_Nfcee_dynpipe_rsp - HCI Timer stopped!!!");
    if (nfa_hci_cb.inst != NFA_HCI_ANY_OK) {
      nfa_hci_cb.hci_state = NFA_HCI_STATE_IDLE;
      evt_data.config_rsp_rcvd.status = status;
      /* Send NFA_HCI_CMD_SENT_EVT to notify failure */
  ...
  if (!pipeId) {
    /* Invalid pipe ID */
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_hci_handle_Nfcee_dynpipe_rsp - Unknown pipe %d", pipeId);
    if (nfa_hci_cb.type == NFA_HCI_COMMAND_TYPE)
  ...
  case NFA_HCI_ANY_OPEN_PIPE:
    ...
    if (!p_pipe) {
      LOG(ERROR) << StringPrintf(
          "nfa_hci_handle_Nfcee_dynpipe_rsp - NULL pipe for PipeId %d", pipeId);
      break;
    }
...
static bool nfa_hci_api_IspipePresent(uint8_t nfceeId, uint8_t gateId) {
  ...
  for (count = 0; count < NFA_HCI_MAX_PIPE_CB; count++) {
    if (((nfa_hci_cb.cfg.dyn_pipes[count].dest_host) == nfceeId) &&
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("nfa_hci_api_IspipePresent -count -- %d !!!", count);
      status = true;
      break;
    }
  }
...
static bool nfa_hci_api_GetpipeId(uint8_t nfceeId, uint8_t gateId,
  ...
  for (count = 0; count < NFA_HCI_MAX_PIPE_CB; count++) {
    if (((nfa_hci_cb.cfg.dyn_pipes[count].dest_host) == nfceeId) &&
        ...
        ((nfa_hci_cb.cfg.dyn_pipes[count].local_gate) == gateId)) {
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("nfa_hci_api_GetpipeId -count -- %d !!!",
...
**
*******************************************************************************/
static void nfa_hci_get_pipe_state_cb(__attribute__((unused)) uint8_t event,
  ...
  while (num_param_id > 0x00) {
    ...
    if (param_id1 == NXP_NFC_SET_CONFIG_PARAM_EXT &&
      ...
      if (status == 1) {
        ...
        if (!nfa_hci_api_IspipePresent(NFA_HCI_HOST_ID_ESE,
                                       NFA_HCI_ETSI12_APDU_GATE)) {
          nfa_hci_update_pipe_status(NFA_HCI_ETSI12_APDU_GATE,
                                     NFA_HCI_APDU_PIPE);
          nfa_hci_cb.IsApduPipeStatusNotCorrect = true;
          if (nfa_hciu_find_gate_by_gid(NFA_HCI_ETSI12_APDU_GATE) == NULL) {
            tNFA_HCI_DYN_GATE *pg;
            int xx;
            for (xx = 0, pg = nfa_hci_cb.cfg.dyn_gates;
                 xx < NFA_HCI_MAX_GATE_CB; xx++, pg++) {
              if (pg->gate_id == 0) {
                ...
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                    "nfa_hci_alloc_apdu_gate id:%d  app_handle: 0x%04x",
