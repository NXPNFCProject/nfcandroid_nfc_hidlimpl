...
void nfa_hci_ee_info_cback(tNFA_EE_DISC_STS status) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_ee_info_cback (): %d", status);
  ...
  case NFA_EE_DISC_STS_OFF:
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_hci_ee_info_cback (): %d  nfa_hci_cb.num_nfcee",
      ...
      for (xx = 0; xx < nfa_hci_cb.num_nfcee; xx++) {
        if ((((nfa_hci_cb.hci_ee_info[xx].ee_interface[0] !=
          ...
          DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              " handle id   -%x", nfa_hci_cb.hci_ee_info[xx].ee_handle);
        }
      }
  ...
  case NFA_EE_MODE_SET_NTF:
    if (nfa_hci_cb.hci_state == NFA_HCI_STATE_NFCEE_ENABLE) {
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("ETSI12 pipe creation configured");
  ...
  case NFA_EE_MODE_SET_COMPLETE:
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_hci_ee_info_cback (): %d  nfa_hci_cb.hci_state",
...
void nfa_hci_proc_nfcc_power_mode(uint8_t nfcc_power_mode) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_hci_proc_nfcc_power_mode () nfcc_power_mode=%d", nfcc_power_mode);
  ...
  if (nfcc_power_mode == NFA_DM_PWR_MODE_FULL) {
    ...
    if (nfa_hci_cb.hci_state == NFA_HCI_STATE_IDLE) {
      ...
      LOG(ERROR) << StringPrintf(
          "nfa_hci_proc_nfcc_power_mode (): Cannot restore now");
...
// NFC-INIT MACH
  ...
  if (nfa_hci_cb.ee_disable_disc) {
    if (nfa_hci_cb.hci_state == NFA_HCI_STATE_STARTUP &&
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("nfa_hci_dh_startup_complete");
      nfa_hci_cb.w4_hci_netwk_init = false;
...
void nfa_hci_startup_complete(tNFA_STATUS status) {
  ...
  if (status == NFA_STATUS_OK) {
    ...
    if (nfcFL.nfccFL._NFC_NXP_STAT_DUAL_UICC_EXT_SWITCH) {
      nfa_hci_cb.hci_state = NFA_HCI_STATE_IDLE;
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("hci_state = NFA_HCI_STATE_IDLE");
...
void nfa_hci_enable_one_nfcee(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_enable_one_nfcee () %d", nfa_hci_cb.num_nfcee);
  ...
  if (nfa_hci_cb.conn_id) {
    if (nfa_sys_is_graceful_disable()) {
      ...
      if (NFC_GetNCIVersion() == NCI_VERSION_1_0) {
        ...
        /* HCI module deregister should be triggered to sto the nfa_sys_main
        timer, so faking a connection close event */
...
void nfa_hci_conn_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  if (event == NFC_CONN_CREATE_CEVT) {
    ...
    if (nfa_ee_connectionClosed())
      ...
      nfa_sys_deregister(NFA_ID_HCI);
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFC_CONN_CLOSE_CEVT handled");
  ...
  if (block == DH_NV_BLOCK) {
    ...
    if ((status != NFA_STATUS_OK) || (!nfa_hci_is_valid_cfg()) ||
      ...
      if (!nfa_hci_is_valid_ese_cfg() &&
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf(" nfa_hci_handle_nv_read; reset ese session");
...
#endif
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_rsp_timeout () State: %u  Cmd: %u",
    ...
    if (nfa_hci_cb.w4_rsp_evt) {
      ...
      if (nfa_ee_nfeeid_active(NFA_HCI_HOST_ID_ESE)) {
        if ((nfa_hci_cb.IsChainedPacket) &&
          ...
          DLOG_IF(INFO, nfc_debug_enabled)
              << StringPrintf("NxpNci: Queue is not empty: %d", msg_len);
...
#if (NXP_EXTNS == TRUE)
static void read_config_timeout_param_values() {
  nfa_hci_cb.max_hci_session_id_read_count = NfcConfig::getUnsigned(
      NAME_NXP_DEFAULT_NFCEE_TIMEOUT, NFA_HCI_MAX_SESSION_ID_RETRY_CNT);
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_hci_init() %d", nfa_hci_cb.max_hci_session_id_read_count);

  nfa_hci_cb.max_nfcee_disc_timeout = NfcConfig::getUnsigned(
      NAME_NXP_DEFAULT_NFCEE_DISC_TIMEOUT, NFA_HCI_NFCEE_DISC_TIMEOUT);
  nfa_hci_cb.max_nfcee_disc_timeout = nfa_hci_cb.max_nfcee_disc_timeout * 1000;
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_init() %d", nfa_hci_cb.max_nfcee_disc_timeout);
}
#endif
/*******************************************************************************
**
        ...
        if (nfcFL.eseFL._JCOP_WA_ENABLE) {
          if (nfa_ee_ce_p61_completed != 0) {
            ...
            if (NFC_GetP61Status((void *)&p61_access_status) < 0) {
              DLOG_IF(INFO, nfc_debug_enabled)
                  << StringPrintf("nfa_hci_evt_hdlr : Check dual mode : "
                                  "NFC_GetP61Status failed");
            } else {
...
void nfa_hci_release_transcieve() {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_hci_release_transcieve (); Release ongoing transcieve");
  if (nfa_hci_cb.hci_state == NFA_HCI_STATE_WAIT_RSP) {
    nfa_sys_stop_timer(&nfa_hci_cb.timer);
    nfa_hci_rsp_timeout();
  }
...
bool nfa_hci_is_valid_ese_cfg(void) {
  ...
  for (xx = 0; xx < NFA_HCI_MAX_PIPE_CB; xx++) {
    if (((nfa_hci_cb.cfg.dyn_pipes[xx].dest_host) == 0xC0) &&
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("nfa_hci_is_valid_ese_cfg()  Pipe id: %u",
                          nfa_hci_cb.cfg.dyn_pipes[xx].pipe_id);
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("nfa_hci_is_valid_ese_cfg()  Pipe state: %u",
