...
void nfa_hci_ee_info_cback(tNFA_EE_DISC_STS status) {
  LOG(ERROR) << StringPrintf(" %d", status);
...
void nfa_hci_enable_one_nfcee(void) {
  ...
  LOG(ERROR) << StringPrintf(" %d", nfa_hci_cb.num_nfcee);
...
bool nfa_hci_enable_one_nfcee(void) {
    ...
    for (xx = nfa_hci_cb.next_nfcee_idx; xx < nfa_hci_cb.num_nfcee; xx++) {
        DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_enable_one_nfcee () %d ee_interface = 0x%x",xx,nfa_hci_cb.ee_info[xx].ee_interface[0]);
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_enable_one_nfcee () enter  hci_enable_state%x",nfa_hci_cb.ee_info[xx].hci_enable_state);
    DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_hci_enable_one_nfcee () enter  nfa_hci_cb.ee_info[xx].ee_status%x",nfa_hci_cb.ee_info[xx].ee_status);
                    ...
                    if(nfceeid == NFA_HCI_FIRST_PROP_HOST){
                      ...
                      if (status != NFA_STATUS_OK) {
                          LOG(ERROR) << StringPrintf(
                              "%s: Power link configuration is unsuccessfull",__func__);
                      }
                    }
                    status = NFC_NfceeModeSet(nfceeid, NFC_MODE_ACTIVATE);
                    if(status == NFA_STATUS_OK) {
...
static void nfa_hci_conn_cback(uint8_t conn_id, tNFC_CONN_EVT event,
 ...
 // DispHcp(p, pkt_len, true);
  ...
  LOG(ERROR) << StringPrintf(" nfa_hci_conn_cback () nfa_hci_cb 1");
