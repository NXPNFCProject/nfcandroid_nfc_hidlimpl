...
tNFA_STATUS NFA_EeGetInfo(uint8_t* p_num_nfcee, tNFA_EE_INFO* p_info) {
  ...
  *p_num_nfcee = 0;
  ...
  LOG(ERROR) << StringPrintf(" num_ret:%d", num_ret);
  *p_num_nfcee = num_ret;
...
tNFA_STATUS NFA_AllEeGetInfo(uint8_t* p_num_nfcee, tNFA_EE_INFO* p_info) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_AllEeGetInfo em_state:%d cur_ee:%d", nfa_ee_cb.em_state,
                   nfa_ee_cb.cur_ee);
  /* validate parameters */
  if (p_info == NULL || p_num_nfcee == NULL) {
  ...
  *p_num_nfcee = 0;
  if (nfa_ee_cb.em_state == NFA_EE_EM_STATE_INIT) {
    LOG(ERROR) << StringPrintf("NFA_AllEeGetInfo bad em state: %d", nfa_ee_cb.em_state);
    return (NFA_STATUS_FAILED);
  }
  ...
  for (xx = 0; (xx < ret) && (num_ret < max_ret); xx++, p_cb++) {
    DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("xx:%d max_ret:%d, num_ret:%d ee_status:0x%x num_interface :%d p_cb->nfcee_id : %d", xx, max_ret,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("num_ret:%d", num_ret);
  *p_num_nfcee = num_ret;
  return (NFA_STATUS_OK);
}
#endif
...
tNFA_STATUS NFA_EeAddAidRouting(tNFA_HANDLE ee_handle, uint8_t aid_len,
  ...
  LOG(ERROR) << StringPrintf(" handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);
#if (NXP_EXTNS == TRUE)
...
tNFA_STATUS NFA_EeRemoveApduPatternRouting(uint8_t apdu_len, uint8_t* p_apdu) {
  ...
  LOG(ERROR) << StringPrintf(" NFA_EeRemoveApduPatternRouting()");
