...
tNFA_STATUS NFA_EeDiscover(tNFA_EE_CBACK *p_cback) {
  ...
  if (nfa_ee_cb.em_state != NFA_EE_EM_STATE_INIT_DONE) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_EeDiscover bad em state: %d", nfa_ee_cb.em_state);
    status = NFA_STATUS_FAILED;
  } else if ((nfa_ee_cb.p_ee_disc_cback != NULL) || (p_cback == NULL)) {
    LOG(ERROR) << StringPrintf(
        "NFA_EeDiscover() in progress or NULL callback function");
...
tNFA_STATUS NFA_EeGetInfo(uint8_t *p_num_nfcee, tNFA_EE_INFO *p_info) {
  ...
  *p_num_nfcee = 0;
  if (nfa_ee_cb.em_state == NFA_EE_EM_STATE_INIT) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_EeGetInfo bad em state: %d", nfa_ee_cb.em_state);
...
tNFA_STATUS NFA_AllEeGetInfo(uint8_t *p_num_nfcee, tNFA_EE_INFO *p_info) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_AllEeGetInfo em_state:%d cur_ee:%d",
  ...
  *p_num_nfcee = 0;
  if (nfa_ee_cb.em_state == NFA_EE_EM_STATE_INIT) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFA_AllEeGetInfo bad em state: %d", nfa_ee_cb.em_state);
    return (NFA_STATUS_FAILED);
  }
  ...
  for (xx = 0; (xx < ret) && (num_ret < max_ret); xx++, p_cb++) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("xx:%d max_ret:%d, num_ret:%d ee_status:0x%x "
                        "num_interface :%d p_cb->nfcee_id : %d",
                        xx, max_ret, num_ret, p_cb->ee_status,
                        p_cb->num_interface, p_cb->nfcee_id);
    if (p_cb->ee_status & NFA_EE_STATUS_INT_MASK) {
      continue;
    }
    p_info->ee_handle = NFA_HANDLE_GROUP_EE | (tNFA_HANDLE)p_cb->nfcee_id;
...
void NFA_setProvisionMode(bool provisionMode) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_setProvisionMode(), provisionMode:%d", provisionMode);
...
tNFA_STATUS NFA_EeDeregister(tNFA_EE_CBACK *p_cback) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeDeregister() %d, status:%d", index, status);
...
tNFA_STATUS NFA_SendPowerLinkCommand(uint8_t nfcee_id, uint8_t cfg_value) {
  ...
  if ((p_msg = (tNFA_EE_API_POWER_LINK_EVT *)GKI_getbuf(
           sizeof(tNFA_EE_API_POWER_LINK_EVT))) != NULL) {
...
tNFA_STATUS NFA_EeModeSet(tNFA_HANDLE ee_handle, tNFA_EE_MD mode) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFA_EeModeSet(): handle:<0x%x>, mode:0x%02X", ee_handle, mode);

  if (p_found == NULL) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("NFA_EeModeSet() invalid NFCEE:0x%04x", ee_handle);
    status = NFA_STATUS_INVALID_PARAM;
  } else {
...
*******************************************************************************/
tNFA_STATUS
NFA_EeSetDefaultTechRouting(tNFA_HANDLE ee_handle,
  ...
  if (p_cb == NULL) {
    ...
    p_msg = (tNFA_EE_API_SET_TECH_CFG *)GKI_getbuf(
        sizeof(tNFA_EE_API_SET_TECH_CFG));
    if (p_msg != NULL) {
...
*******************************************************************************/
tNFA_STATUS
NFA_EeSetDefaultProtoRouting(tNFA_HANDLE ee_handle,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeSetDefaultProtoRouting():\
...
tNFA_STATUS NFA_EeAddAidRouting(tNFA_HANDLE ee_handle, uint8_t aid_len,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeAddAidRouting(): handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);
...
*******************************************************************************/
tNFA_STATUS
NFA_EeAddApduPatternRouting(uint8_t apdu_data_len, uint8_t *apdu_data,
                            uint8_t apdu_mask_len, uint8_t *apdu_mask,
                            tNFA_HANDLE ee_handle, uint8_t power_state) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeAddApduRouting(): handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);
  if (p_cb == NULL || apdu_data_len == 0 || apdu_mask_len == 0 ||
      apdu_data == NULL || apdu_mask == NULL ||
      apdu_data_len > NFC_MAX_APDU_DATA_LEN ||
...
tNFA_STATUS NFA_AddEePowerState(tNFA_HANDLE ee_handle,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_AddEePowerState(): handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);
...
tNFA_STATUS NFA_EeRemoveApduPatternRouting(uint8_t apdu_len, uint8_t *p_apdu) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << __func__;
  if ((apdu_len == 0) || (p_apdu == NULL) ||
      (apdu_len > NFC_MAX_APDU_DATA_LEN)) {
...
tNFA_STATUS NFA_EeRemoveSystemCodeRouting(uint16_t systemcode) {
  ...
  if (systemcode == 0) {
    ...
    tNFA_EE_API_REMOVE_SYSCODE *p_msg =
        (tNFA_EE_API_REMOVE_SYSCODE *)GKI_getbuf(
...
tNFA_STATUS NFA_EeConnect(tNFA_HANDLE ee_handle, uint8_t ee_interface,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeConnect(): handle:<0x%x> ee_interface:0x%x",
...
tNFA_STATUS NFA_EeSendData(tNFA_HANDLE ee_handle, uint16_t data_len,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeSendData(): handle:<0x%x>", ee_handle);
...
tNFA_STATUS NFA_EeDisconnect(tNFA_HANDLE ee_handle) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeDisconnect(): handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);
  ...
  if ((p_cb == NULL) || (p_cb->conn_st != NFA_EE_CONN_ST_CONN)) {
    ...
    p_msg =
        (tNFA_EE_API_DISCONNECT *)GKI_getbuf(sizeof(tNFA_EE_API_DISCONNECT));
    if (p_msg != NULL) {
