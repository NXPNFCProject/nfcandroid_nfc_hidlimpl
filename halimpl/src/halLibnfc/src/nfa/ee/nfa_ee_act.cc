...
static void nfa_ee_trace_apdu(std::string p_str, tNFA_EE_API_ADD_APDU* p_apdu) {
  ...
  for (xx = 0; xx < mask_len; xx++) {
   ...
   LOG(ERROR) << StringPrintf(" %s id:0x%x apdu_len=%d apdu:%s mask_len=%d mask:%s",
...
void nfa_ee_api_deregister(tNFA_EE_MSG* p_data) {
  ...
  LOG(ERROR) << StringPrintf(" nfa_ee_api_deregister");
...
void nfa_ee_api_add_apdu(tNFA_EE_MSG* p_data) {
  ...
  LOG(ERROR) << StringPrintf(" Enter : nfa_ee_api_add_apdu");
  p_chk_cb =
...
void nfa_ee_api_remove_apdu(tNFA_EE_MSG* p_data) {
  ...
  if (p_cb && p_cb->apdu_pattern_entries) {
   LOG(ERROR) << StringPrintf(" apdu_rt_info[%d]: 0x%02x", entry,
   ...
   /* mark routing and VS changed */
    ...
    if ((entry + 1) < p_cb->apdu_pattern_entries) {
      ...
      LOG(ERROR) << StringPrintf(" nfa_ee_api_remove_apdu len:%d, rest_len:%d", len,
                       rest_len);
...
void nfa_ee_get_tech_route(uint8_t power_state, uint8_t* p_handles) {
  ...
  LOG(ERROR) << StringPrintf(" %d", power_state);
