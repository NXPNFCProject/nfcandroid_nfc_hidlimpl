...
const uint8_t nfa_ee_proto_list[NFA_EE_NUM_PROTO] = {NFC_PROTOCOL_T1T,
                                                     NFC_PROTOCOL_T2T,
                                                     NFC_PROTOCOL_T3T,
...
uint8_t NFA_REMOVE_ALL_APDU[] = {0xFF, 0xFF, 0xFF, 0xFF,
                                 0xFF, 0xFF, 0xFF, 0xEF};
...
static void nfa_ee_trace_apdu(std::string p_str, tNFA_EE_API_ADD_APDU *p_apdu) {
  ...
  if (apdu_len > NFA_MAX_APDU_DATA_LEN) {
    LOG(ERROR) << StringPrintf("apdu_len: %d exceeds max(%d)", apdu_len,
                               NFA_MAX_APDU_DATA_LEN);
    apdu_len = NFA_MAX_APDU_DATA_LEN;
    p_apdu->apdu_len = apdu_len;
  }

  if (mask_len > NFA_MAX_APDU_MASK_LEN) {
    LOG(ERROR) << StringPrintf("mask_len: %d exceeds max(%d)", mask_len,
                               NFA_MAX_APDU_MASK_LEN);
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s id:0x%x apdu_len=%d apdu:%s mask_len=%d mask:%s", p_str.c_str(),
      p_apdu->nfcee_id, apdu_len, apdu, mask_len, mask);
}
/*******************************************************************************
**
    ...
    for (int xx = 0; xx < p_cb->aid_entries; xx++) {
      ...
      if (p_cb->aid_rt_info[xx] & NFA_EE_AE_ROUTE) {
        ...
        if (nfa_ee_is_active(p_cb->aid_rt_loc[xx] | NFA_HANDLE_GROUP_EE)) {
          add_route_aid_tlv(&pp, pa, p_cb->aid_rt_loc[xx],
                            p_cb->aid_pwr_cfg[xx], tag);
          num_tlv++;
        } else {
          DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s -  ignoring route loc%x", __func__, p_cb->aid_rt_loc[xx]);
        }
#else
...
static void nfa_ee_add_apdu_route_to_ecb(tNFA_EE_ECB *p_cb, uint8_t *pp,
  ...
  if (p_cb->apdu_pattern_entries) {
    ...
    for (xx = 0; xx < p_cb->apdu_pattern_entries; xx++) {
      ...
      if (p_cb->apdu_rt_info[xx] & NFA_EE_AE_ROUTE) {
        ...
        *pp++ = (p_cb->apdu_rt_info[xx] >> NFA_EE_APDU_ROUTE_MASK);
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "nfa_ee_route_add_one_ecb_by_route_order p_cb->apdu_rt_info[xx] %x",
            p_cb->apdu_rt_info[xx]);
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "nfa_ee_route_add_one_ecb_by_route_order --before num_tlv:- %d",
          num_tlv);
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "nfa_ee_route_add_one_ecb_by_route_order --before new_size:- %d",
          new_size);
      nfa_ee_check_set_routing(new_size, p_max_len, ps, p_cur_offset);
      if (*ps == 0) {
        /* just sent routing command, update local */
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_ee_route_add_one_ecb_by_route_order --new_size:- %d", new_size);
  }
}
/*******************************************************************************
**
...
nfa_ee_add_sys_code_route_to_ecb(tNFA_EE_ECB *p_cb, uint8_t *pp, uint8_t *p,
                                 uint8_t *p_buff, int *p_cur_offset,
                                 int *p_max_len) {
...
int nfa_all_ee_find_total_apdu_pattern_len() {
  ...
  for (xx = 0; xx < NFA_EE_MAX_EE_SUPPORTED; xx++, p_cb++) {
    ...
    total_len +=
        (p_cb->apdu_pattern_entries * 2); /*Adding nfceeid/power_state */
  }
  return total_len;
}
#endif
...
tNFA_STATUS nfa_ee_get_num_nfcee_configured(tNFA_VSC_CBACK *p_cback) {
  ...
  *parm_len = (p - num_param);
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ee_get_num_nfcee_configured %x", *num_param);
...
**
*********************************************************************************/
void nfa_ee_read_num_nfcee_config_cb(__attribute__((unused)) uint8_t event,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_ee_read_num_nfcee_config_cb %x", configured_num_nfcee);
}
...
void nfa_ee_api_discover(tNFA_EE_MSG *p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_ee_api_discover() in_use:%d", nfa_ee_cb.discv_timer.in_use);
...
void nfa_ee_api_register(tNFA_EE_MSG *p_data) {
  ...
  if ((nfcFL.chipType != pn547C2) &&
    ...
    /* Dynamic ecb allocation based on max aid entry calculated from max.
     * routing table size( from core_init rsp )*/
...
void nfa_ee_api_mode_set(tNFA_EE_MSG *p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ee_api_mode_set() handle:0x%02x mode:%d",
  ...
  if (p_chk_cb) {
    ...
    if (p_chk_cb == p_cb) {
      ...
      if (new_size > NFC_GetLmrtSize()) {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("Exceed LMRT size:%d (add ROUTE)", new_size);
        evt_data.status = NFA_STATUS_BUFFER_FULL;
        p_cb->aid_rt_info[entry] &= ~NFA_EE_AE_ROUTE;
      } else {
...
/* Find the total length so far */
...
#if (NXP_EXTNS == TRUE)
      LOG(ERROR) << StringPrintf("Exceed capacity: (len_needed:%d + len:%d) > "
...
#endif
    {
      ...
      if (new_size > NFC_GetLmrtSize()) {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("Exceed LMRT size:%d", new_size);
        evt_data.status = NFA_STATUS_BUFFER_FULL;
      } else {
/* add AID */
#if (NXP_EXTNS == TRUE)
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "Exceed NFA_EE_MAX_AID_ENTRIES:%d", NFA_EE_MAX_AID_ENTRIES);
      evt_data.status = NFA_STATUS_BUFFER_FULL;
    }
  }
  ...
  if (p_cb && p_cb->aid_entries) {
    ...
    if ((entry + 1) < p_cb->aid_entries) {
      ...
      GKI_shiftup(&p_cb->aid_pwr_cfg[entry], &p_cb->aid_pwr_cfg[entry + 1],
                  rest_len);
      GKI_shiftup(&p_cb->aid_info[entry], &p_cb->aid_info[entry + 1], rest_len);
  ...
  if (p_chk_cb) {
    ...
    if (p_chk_cb == p_cb) {
      ...
      if (((nfcFL.chipType != pn547C2) &&
           ...
           (new_size > max_aid_config_length)) ||
          ((nfcFL.chipType == pn547C2) &&
           (!nfcFL.nfcMwFL._NFC_NXP_AID_MAX_SIZE_DYN) &&
           (new_size > NFC_GetLmrtSize()))) {
#else
      if (new_size > NFC_GetLmrtSize()) {
#endif
        DLOG_IF(INFO, nfc_debug_enabled)
...
/* Find the total length so far */
    ...
    /* make sure the control block has enough room to hold this entry */
    len_needed = p_add->apdu_len + p_add->mask_len + 2 +
                 2; /*tag/len/apdu_mask/apdu/power_state/nfceeid*/
    ...
    if ((len_needed + aid_len + apdu_len) >
            ...
            NFC_GetLmrtSize()
#endif
        || ((dh_ecb->size_apdu + p_add->apdu_len + p_add->mask_len + 2) >=
            NFA_EE_TOTAL_APDU_PATTERN_SIZE)) {
#if (NXP_EXTNS == TRUE)
      if ((nfcFL.chipType != pn547C2) &&
          nfcFL.nfcMwFL._NFC_NXP_AID_MAX_SIZE_DYN) {
        LOG(ERROR) << StringPrintf("Exceed capacity: (len_needed:%d + len:%d) "
...
/* add APDU */
      ...
      *p++ = p_add->apdu_len + p_add->mask_len;
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("Exceed NFA_EE_MAX_APDU_PATTERN_ENTRIES:%d",
                          NFA_EE_MAX_APDU_PATTERN_ENTRIES);
      evt_data.status = NFA_STATUS_BUFFER_FULL;
    }
  }
...
void nfa_ee_api_remove_apdu(tNFA_EE_MSG *p_data) {
  ...
  p_cb = nfa_ee_find_apdu_offset(p_data->rm_apdu.apdu_len,
                                 p_data->rm_apdu.p_apdu, &offset, &entry);
  if (p_cb && p_cb->apdu_pattern_entries) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "apdu_rt_info[%d]: 0x%02x", entry, p_cb->apdu_rt_info[entry]);
    /* mark routing and VS changed */
    ...
    if ((entry + 1) < p_cb->apdu_pattern_entries) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "nfa_ee_api_remove_apdu len:%d, rest_len:%d", len, rest_len);
  ...
  /*Clear All APDUs*/
    ...
    uint32_t xx;
    for (xx = 0; xx < NFA_EE_NUM_ECBS; xx++) {
      memset(&nfa_ee_cb.ecb[xx].apdu_cfg[0], 0x00,
             sizeof(nfa_ee_cb.ecb[xx].apdu_cfg));
      memset(&nfa_ee_cb.ecb[xx].apdu_len[0], 0x00,
             NFA_EE_MAX_APDU_PATTERN_ENTRIES);
      memset(&nfa_ee_cb.ecb[xx].apdu_pwr_cfg[0], 0x00,
             NFA_EE_MAX_APDU_PATTERN_ENTRIES);
      memset(&nfa_ee_cb.ecb[xx].apdu_rt_info[0], 0x00,
             NFA_EE_MAX_APDU_PATTERN_ENTRIES);
      memset(&nfa_ee_cb.ecb[xx].apdu_rt_loc[0], 0x00,
             NFA_EE_MAX_APDU_PATTERN_ENTRIES);
...
void nfa_ee_api_add_sys_code(tNFA_EE_MSG *p_data) {
  ...
  if (p_chk_cb) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: The SC entry already registered "
...
static void nfa_ee_remove_pending(void) {
  ...
  for (xx = 0; xx < nfa_ee_cb.cur_ee; xx++, p_cb++) {
    if ((p_cb->nfcee_id != NFA_EE_INVALID) &&
      ...
      num_removed++;
      if (first_removed == nfcFL.nfccFL._NFA_EE_MAX_EE_SUPPORTED)
        first_removed = xx;
    }
  }
...
void nfa_ee_nci_disc_rsp(tNFA_EE_MSG *p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_ee_nci_disc_rsp() em_state:%d cur_ee:%d, num_nfcee:%d",
...
void nfa_ee_nci_disc_ntf(tNFA_EE_MSG *p_data) {
  ...
  if (p_cb) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_ee_cb.p_ee_disc_cback : %p notify_new_ee :%d "
                        "nfa_dm_is_active() : %d p_cb->ee_status : %d",
                        nfa_ee_cb.p_ee_disc_cback, notify_new_ee,
...
void nfa_ee_nci_pwr_link_ctrl_rsp(tNFA_EE_MSG *p_data) {
  ...
  if (nfcFL.eseFL._WIRED_MODE_STANDBY != true) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf(" WIRED_MODE_STANDBY() not available. Returning");
    return;
  }
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      " nfa_ee_nci_pwr_link_ctrl_rsp: status = %d ", pwr_lnk_ctrl.status);
...
void nfa_ee_nci_set_mode_info(tNFA_EE_MSG *p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      " nfa_ee_nci_set_mode_info: status = %d ", ee_set_mode_info.status);
  if (nfa_ee_cb.p_enable_cback)
...
void nfa_ee_nci_mode_set_rsp(tNFA_EE_MSG *p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_ee_nci_mode_set_rsp() handle:0x%02x mode:%d statu : %d",
...
/* Do not update routing table on secure element enable/disable. */
      ...
      if (p_cb->tech_switch_on | p_cb->tech_switch_off |
          ...
          p_cb->proto_switch_off | p_cb->proto_battery_off | p_cb->aid_entries |
          p_cb->apdu_pattern_entries) {
  ...
  for (xx = 0; xx < p_cbk->num_info; xx++) {
    ...
    if (!p_cb) {
      ...
      if (p_cb) {
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "Cannot allocate cb for NFCEE: 0x%x", p_cbk->info[xx].nfcee_id);
        continue;
      }
    } else {
...
void nfa_ee_get_tech_route(uint8_t power_state, uint8_t *p_handles) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ee_get_tech_route(): power state : %d", power_state);
...
void nfa_ee_route_add_one_ecb_by_route_order(tNFA_EE_ECB *p_cb, int rout_type,
  ...
  case NCI_ROUTE_ORDER_SYS_CODE: {
    nfa_ee_add_sys_code_route_to_ecb(p_cb, pp, p, ps, p_cur_offset, p_max_len);
  } break;
  default: {
    DLOG_IF(INFO, nfc_debug_enabled)
...
static bool nfa_ee_need_recfg(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ee_need_recfg() ee_cfged: 0x%02x ee_cfg_sts: 0x%02x",
  ...
  if (nfa_ee_cb.ee_cfged || nfa_ee_cb.ee_cfg_sts) {
    if (nfa_ee_cb.ee_cfg_sts & NFA_EE_STS_CHANGED) {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s:Routing table dirty, update required...", __func__);
      needed = true;
    } else {
      p_cb = &nfa_ee_cb.ecb[NFA_EE_CB_4_DH];
...
void nfa_ee_lmrt_to_nfcc(__attribute__((unused)) tNFA_EE_MSG *p_data) {
  ...
  if ((nfcFL.chipType != pn547C2) &&
      (nfcFL.nfcMwFL._NFC_NXP_AID_MAX_SIZE_DYN == true)) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "max supported routing table size: %d", max_routing_table_size);
  ...
  if (p == NULL) {
    LOG(ERROR) << StringPrintf(
        "nfa_ee_lmrt_to_nfcc() no buffer to send routing info.");
#if (NXP_EXTNS == TRUE)
  ...
  *p = 0;
  for (int rt = NCI_ROUTE_ORDER_AID; rt <= NCI_ROUTE_ORDER_TECHNOLOGY; rt++) {
    ...
    if (rt == NCI_ROUTE_ORDER_TECHNOLOGY)
      more = false;
...
void find_and_resolve_tech_conflict() {
  if (nfcFL.chipType == pn547C2) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: chipType : pn547C2. Returning", __func__);
  }
  int yy, xx;
  tNFA_EE_ECB *p_cb = nfa_ee_cb.ecb;
  ...
  if (NfcConfig::hasKey(NAME_DEFAULT_OFFHOST_ROUTE)) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "%s:NXP_DEFAULT_OFFHOST_ROUTE=0x0%lu;", __func__, preferred_se);
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s:techF_found=0x%x,techF_ee= 0x%x;", __func__, techF_found, techF_ee);
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s:techA_found=0x%x,techA_ee= 0x%x;", __func__, techA_found, techA_ee);
...
uint16_t nfa_ee_lmrt_size() {
  ...
  if ((nfcFL.chipType != pn547C2) &&
    ...
    return len < NFA_EE_MAX_AID_CFG_LEN_STAT ? len
                                             : NFA_EE_MAX_AID_CFG_LEN_STAT;
  }
}
...
uint16_t nfa_ee_find_max_aid_config_length() {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ee_find_max_aid_config_length");
  /**
  ...
  if ((nfcFL.chipType != pn547C2) &&
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("Not allowed for chip type. Returning");
    return 0;
  }
}
...
uint16_t nfa_ee_api_get_max_aid_config_length() {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_ee_api_get_max_aid_config_length");
  if ((nfcFL.chipType != pn547C2) &&
      (nfcFL.nfcMwFL._NFC_NXP_AID_MAX_SIZE_DYN == true)) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "nfa_ee_api_get_max_aid_config_length: %d", max_aid_config_length);
