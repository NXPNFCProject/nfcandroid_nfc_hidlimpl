...
typedef struct {
   ...
   * apdu_cfg[] contains APDU and APDU mask and maybe some other VS information
   * in TLV format The first T is always NFA_EE_APDU_CFG_TAG_NAME, the L is the
   * actual APDU length and mask length the apdu_len is the total length of all
   * the TLVs associated with this AID entry
   */
  uint8_t apdu_len[NFA_EE_MAX_APDU_PATTERN_ENTRIES]; /* the actual lengths in
                                                        apdu_cfg */
  uint8_t apdu_pwr_cfg[NFA_EE_MAX_APDU_PATTERN_ENTRIES]; /* power configuration
                                                            of this APDU Pattern
                                                            entry */
  uint8_t
      apdu_rt_info[NFA_EE_MAX_APDU_PATTERN_ENTRIES]; /* route/vs info for this
                                                        APDU PATTERN entry */
  uint8_t apdu_rt_loc[NFA_EE_MAX_APDU_PATTERN_ENTRIES]; /* route location info
                                                           for this APDU Pattern
                                                           entry */
  uint8_t apdu_cfg[NFA_EE_TOTAL_APDU_PATTERN_SIZE]; /* routing entries based on
                                                       APDU PATTERN */
  uint8_t
      apdu_pattern_entries; /* The number of APDU PATTERN entries in aid_cfg */
 ...
 */
  ...
  uint8_t aid_rt_loc_stat[NFA_EE_MAX_AID_ENTRIES]; /* route location info for
                                                    this AID entry */
  uint8_t
      aid_len_stat[NFA_EE_MAX_AID_ENTRIES]; /* the actual lengths in aid_cfg */
  uint8_t aid_pwr_cfg_stat[NFA_EE_MAX_AID_ENTRIES]; /* power configuration of
                                                     this AID entry */
  uint8_t aid_rt_info_stat[NFA_EE_MAX_AID_ENTRIES]; /* route/vs info for this
                                                     AID entry */
  uint8_t
      aid_cfg_stat[NFA_EE_MAX_AID_CFG_LEN]; /* routing entries based on AID */
#else
