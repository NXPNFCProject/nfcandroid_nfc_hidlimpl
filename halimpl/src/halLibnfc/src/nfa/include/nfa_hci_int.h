...
typedef struct {
  ...
  uint32_t max_hci_session_id_read_count; /*Count for maximum  session id retry
                                             value */
  uint32_t max_nfcee_disc_timeout; /*Config file timeout value for all the NFCEE
                                      to be discovered */
...
nfa_hciu_find_active_pipe_by_owner(tNFA_HANDLE app_handle);
...
extern void
nfa_hciu_send_to_apps_handling_connectivity_evts(tNFA_HCI_EVT event,
                                                 tNFA_HCI_EVT_DATA *p_evt);
