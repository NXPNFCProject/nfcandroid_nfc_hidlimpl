...
void NfcAdaptation::Initialize() {
  ...
  if (NfcConfig::hasKey(NAME_NFA_DM_CFG)) {
    ...
    if (dm_config.size() > 0)
      nfa_dm_cfg.auto_detect_ndef = dm_config[0];
    if (dm_config.size() > 1)
      nfa_dm_cfg.auto_read_ndef = dm_config[1];
    if (dm_config.size() > 2)
      nfa_dm_cfg.auto_presence_check = dm_config[2];
    if (dm_config.size() > 3)
      nfa_dm_cfg.presence_check_option = dm_config[3];
  ...
  if (NfcConfig::hasKey(NAME_NFA_PROPRIETARY_CFG)) {
    ...
    if (p_config.size() > 5)
      nfa_proprietary_cfg.pro_protocol_mfc = p_config[5];
    if (p_config.size() > 6)
...
**
*******************************************************************************/
int NfcAdaptation::HalGetFwDwnldFlag(__attribute__((unused))
...
void NfcAdaptation::DownloadFirmware() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s: send CORE_INIT NCI Version : %d", func, NFA_GetNCIVersion());
  if (NFA_GetNCIVersion() == NCI_VERSION_2_0)
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "fw_update required  -> %d", fw_update_inf.fw_update_reqd);
  if (fw_update_inf.fw_update_reqd == true) {
    ...
    if (fw_dwnld_status != NFC_STATUS_OK) {
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: FW Download failed", func);
    } else {
      isSignaled = SIGNAL_NONE;
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: send CORE_RESET", func);
      HalWrite(cmd_reset_nci_size, cmd_reset_nci);
      mHalCoreResetCompletedEvent.lock();
      if (SIGNAL_NONE == isSignaled) {
        mHalCoreResetCompletedEvent.wait();
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: send CORE_INIT", func);
      if (NFA_GetNCIVersion() == NCI_VERSION_2_0)
        HalWrite(cmd_init_nci_size, cmd_init_nci2_0);
      else
        HalWrite(cmd_init_nci_size, cmd_init_nci);
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: try init HAL", func);
      isSignaled = SIGNAL_NONE;
...
void NfcAdaptation::HalDownloadFirmwareCallback(nfc_event_t event,
  ...
  case HAL_NFC_POST_INIT_CPLT_EVT: {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: HAL_NFC_POST_INIT_CPLT_EVT", func);
#if (NXP_EXTNS == TRUE)
