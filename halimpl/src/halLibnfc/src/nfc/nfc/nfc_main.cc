  ...
  if (nfc_status == NCI_STATUS_OK) {
    ...
    if (nfc_cb.nci_version == NCI_VERSION_2_0) {
      if (evt_data.enable.nci_features & NCI_FEAT_HCI_NETWORK) {
        ...
        p += 2; /* skip the data buffer size and data credit for static HCI
                   connection */
      }
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfc_cb.num_disc_maps = %d", nfc_cb.num_disc_maps);
...
void nfc_main_handle_hal_evt(tNFC_HAL_EVT_MSG *p_msg) {
  uint8_t *ps;
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfc_main_handle_hal_evt(): HAL event=0x%x", p_msg->hal_evt);
  ...
  default:
    LOG(ERROR) << StringPrintf(
        "nfc_main_handle_hal_evt (): unhandled event (0x%x).", p_msg->hal_evt);
    break;
  }
}
...
void nfc_main_flush_cmd_queue(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfc_main_flush_cmd_queue ()");
...
void NFC_Disable(void) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFC_Disable (): nfc_state = %d", nfc_cb.nfc_state);
...
tNFC_STATUS NFC_DiscoveryMap(uint8_t num, tNFC_DISCOVER_MAPS *p_maps,
  ...
  num_intf = 0;
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nci_interfaces supported by NFCC: 0x%x num = 0x%x",
  ...
  for (xx = 0; xx < num_disc_maps; xx++) {
    ...
    if (is_supported) {
      if (num_intf < (NFC_NFCC_MAX_NUM_VS_INTERFACE + NCI_INTERFACE_MAX)) {
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("num_intf exeeds the limit 0x%02x",
  ...
  for (xx = 0; xx < NFC_NFCC_MAX_NUM_VS_INTERFACE + NCI_INTERFACE_MAX; xx++) {
    ...
    LOG(WARNING) << StringPrintf("max_maps[%d].mode = 0x%2x", xx,
                                 max_maps[xx].mode);
...
tNFC_STATUS NFC_SetPowerOffSleep(bool enable) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFC_SetPowerOffSleep () enable = %d", enable);
...
bool NFC_IsLowRamDevice() {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFC_IsLowRamDevice: %s", nfc_cb.isLowRam ? "true" : "false");
...
void NFC_SetStaticHciCback(tNFC_CONN_CBACK *p_cback) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NFC_SetStaticHciCback () evt_data.conn_create.dest_type  = %d",
      NCI_DEST_TYPE_NFCEE);
  ...
  if (p_cback && p_cb->buff_size && p_cb->num_buff) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "NFC_SetStaticHciCback 2 () evt_data.conn_create.dest_type  = %d",
        NCI_DEST_TYPE_NFCEE);
 ...
 **
 *******************************************************************************/
tNFC_chipType NFC_GetChipType() { return chipType; }
