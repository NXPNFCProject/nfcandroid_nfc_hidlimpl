...
static bool rw_t4t_send_to_lower(NFC_HDR *p_c_apdu) {
  if (NFC_SendData(NFC_RF_CONN_ID, p_c_apdu) != NFC_STATUS_OK) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_send_to_lower (): NFC_SendData () failed");
    return false;
  }
...
static bool rw_t4t_get_hw_version(void) {
  ...
  if (!p_c_apdu) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_get_hw_version (): Cannot allocate buffer");
    return false;
  }
...
static bool rw_t4t_get_sw_version(void) {
  ...
  if (!p_c_apdu) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_get_sw_version (): Cannot allocate buffer");
    return false;
  }
...
static bool rw_t4t_get_uid_details(void) {
  ...
  if (!p_c_apdu) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_get_uid_details (): Cannot allocate buffer");
    return false;
  }
...
static bool rw_t4t_create_ccfile(void) {
  ...
  if (!p_c_apdu) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_create_ccfile (): Cannot allocate buffer");
    return false;
  }
...
static bool rw_t4t_select_file(uint16_t file_id) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t4t_select_file (): File ID:0x%04X", file_id);
...
static bool rw_t4t_read_file(uint16_t offset, uint16_t length,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "rw_t4t_read_file () offset:%d, length:%d, is_continue:%d, ", offset,
      length, is_continue);
...
static bool rw_t4t_update_nlen(uint16_t ndef_len) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t4t_update_nlen () NLEN:%d", ndef_len);
...
static bool rw_t4t_update_file(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t4t_update_file () rw_offset:%d, rw_length:%d",
...
static bool rw_t4t_update_cc_to_readonly(void) {
  ...
  if (!p_c_apdu) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_update_cc_to_readonly (): Cannot allocate buffer");
    return false;
  }
...
static bool rw_t4t_select_application(uint8_t version) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t4t_select_application () version:0x%X", version);

  p_c_apdu = (NFC_HDR *)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_c_apdu) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_select_application (): Cannot allocate buffer");
    return false;
  }
...
static bool rw_t4t_validate_cc_file(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t4t_validate_cc_file ()");

  if (p_t4t->cc_file.cclen < T4T_CC_FILE_MIN_LEN) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_validate_cc_file (): CCLEN (%d) is too short",
        p_t4t->cc_file.cclen);
    return false;
  }
  ...
  if (p_t4t->cc_file.max_le < 0x000F) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_validate_cc_file (): MaxLe (%d) is too small",
  ...
  if (p_t4t->cc_file.max_lc < 0x0001) {
    LOG(ERROR) << StringPrintf(
        "rw_t4t_validate_cc_file (): MaxLc (%d) is too small",
  ...
  if ((p_t4t->cc_file.ndef_fc.file_id == T4T_CC_FILE_ID) ||
    ...
    LOG(ERROR) << StringPrintf(
        "rw_t4t_validate_cc_file (): File ID (0x%04X) is invalid",
...
static void rw_t4t_sm_ndef_format(NFC_HDR *p_r_apdu) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "rw_t4t_sm_ndef_format (): sub_state:%s (%d)",
...
static void rw_t3Bt_sm_get_card_id(NFC_HDR *p_r_apdu) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "rw_t3Bt_sm_get_id (): sub_state:%s (%d)",
      rw_t4t_get_sub_state_name(p_t4t->sub_state).c_str(), p_t4t->sub_state);
...
static void rw_t4t_sm_detect_ndef(NFC_HDR *p_r_apdu) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "rw_t4t_sm_detect_ndef (): sub_state:%s (%d)",
      rw_t4t_get_sub_state_name(p_t4t->sub_state).c_str(), p_t4t->sub_state);
  ...
  if (status_words != T4T_RSP_CMD_CMPLTED) {
    ...
    if ((p_t4t->sub_state == RW_T4T_SUBSTATE_WAIT_SELECT_APP) &&
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("rw_t4t_sm_detect_ndef (): retry with version=0x%02X",
                          p_t4t->version);
...
static void rw_t4t_sm_read_ndef(NFC_HDR *p_r_apdu) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "rw_t4t_sm_read_ndef (): sub_state:%s (%d)",
      rw_t4t_get_sub_state_name(p_t4t->sub_state).c_str(), p_t4t->sub_state);
...
static void rw_t4t_sm_update_ndef(NFC_HDR *p_r_apdu) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "rw_t4t_sm_update_ndef (): sub_state:%s (%d)",
      rw_t4t_get_sub_state_name(p_t4t->sub_state).c_str(), p_t4t->sub_state);
  ...
  default:
    LOG(ERROR) << StringPrintf(
        "rw_t4t_sm_update_ndef (): unknown sub_state = %d", p_t4t->sub_state);
    rw_t4t_handle_error(NFC_STATUS_FAILED, 0, 0);
    break;
  }
}
...
static void rw_t4t_sm_set_readonly(NFC_HDR *p_r_apdu) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "rw_t4t_sm_set_readonly (): sub_state:%s (%d)",
      rw_t4t_get_sub_state_name(p_t4t->sub_state).c_str(), p_t4t->sub_state);
  ...
  case RW_T4T_SUBSTATE_WAIT_SELECT_NDEF_FILE:
    ...
    if (rw_cb.p_cback) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "rw_t4t_sm_set_readonly (): Sent RW_T4T_SET_TO_RO_EVT");
  ...
  default:
    LOG(ERROR) << StringPrintf(
        "rw_t4t_sm_set_readonly (): unknown sub_state = %d", p_t4t->sub_state);
    rw_t4t_handle_error(NFC_STATUS_FAILED, 0, 0);
    break;
  }
}
...
void rw_t4t_process_timeout(TIMER_LIST_ENT *p_tle) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t4t_process_timeout () event=%d", p_tle->event);

  if (p_tle->event == NFC_TTYPE_RW_T4T_RESPONSE) {
    rw_t4t_handle_error(NFC_STATUS_TIMEOUT, 0, 0);
  } else {
    LOG(ERROR) << StringPrintf("rw_t4t_process_timeout () unknown event=%d",
                               p_tle->event);
  }
}
...
static void rw_t4t_data_cback(__attribute__((unused)) uint8_t conn_id,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t4t_data_cback () event = 0x%X", event);
  nfc_stop_quick_timer(&p_t4t->timer);
  ...
  case NFC_RF_WTX_CEVT:
    if (p_t4t->state == RW_T4T_STATE_IDLE) {
      ...
      nfc_start_quick_timer(&p_t4t->timer, NFC_TTYPE_RW_T4T_RESPONSE,
                            (RW_T4T_TOUT_RESP * QUICK_TIMER_TICKS_PER_SEC) /
                                1000);
    }
    return;
#endif
...
#endif
  default:
    LOG(ERROR) << StringPrintf("rw_t4t_data_cback (): invalid state=%d",
                               p_t4t->state);
    GKI_freebuf(p_r_apdu);
    break;
  }
...
tNFC_STATUS RW_T4tReadNDef(void) {
  ...
  if (rw_cb.tcb.t4t.state != RW_T4T_STATE_IDLE) {
    LOG(ERROR) << StringPrintf(
        "RW_T4tReadNDef ():Unable to start command at state (0x%X)",
        rw_cb.tcb.t4t.state);
    return NFC_STATUS_FAILED;
  }
...
tNFC_STATUS RW_T4tUpdateNDef(uint16_t length, uint8_t *p_data) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("RW_T4tUpdateNDef () length:%d", length);
...
tNFC_STATUS RW_T4tPresenceCheck(uint8_t option) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("RW_T4tPresenceCheck () %d", option);
...
tNFC_STATUS RW_T4tSetNDefReadOnly(void) {
  ...
  if (rw_cb.tcb.t4t.ndef_status & RW_T4T_NDEF_STATUS_NDEF_DETECTED) {
    ...
    if (rw_cb.tcb.t4t.ndef_status & RW_T4T_NDEF_STATUS_NDEF_READ_ONLY) {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "RW_T4tSetNDefReadOnly (): NDEF is already read-only");
