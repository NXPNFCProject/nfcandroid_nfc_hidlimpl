    ...
    if (p_data) {
      ...
      if (status == NFC_STATUS_OK) {
        nfc_start_quick_timer(&p_t1t->timer, NFC_TTYPE_RW_T1T_RESPONSE,
                              (RW_T1T_TOUT_RESP * QUICK_TIMER_TICKS_PER_SEC) /
                                  1000);
      }
    } else {
...
static tRW_EVENT rw_t1t_handle_rid_rsp(NFC_HDR *p_pkt) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t1t_handle_rid_rsp (): UID0-3=%02x%02x%02x%02x",
...
static void rw_t1t_process_error(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t1t_process_error () State: %u", p_t1t->state);
