...
static void rw_t2t_handle_ndef_write_rsp(uint8_t *p_data) {
  ...
  case RW_T2T_SUBSTATE_WAIT_READ_TERM_TLV_BLOCK:
    ...
    offset = (uint8_t)(
        ((p_t2t->terminator_byte_index / T2T_BLOCK_SIZE) - p_t2t->block_read) *
        T2T_BLOCK_SIZE);
  ...
  case RW_T2T_SUBSTATE_WAIT_WRITE_NDEF_LEN_NEXT_BLOCK:
    if (rw_t2t_is_read_before_write_block((uint16_t)(p_t2t->block_written + 1),
                                          &block) == true) {
  ...
  case RW_T2T_SUBSTATE_WAIT_READ_NDEF_LEN_BLOCK:
    ...
    /* Update the block with new NDEF Message */
    if (rw_t2t_write_ndef_next_block(p_t2t->block_read, p_t2t->new_ndef_msg_len,
                                     true) == NFC_STATUS_OK)
...
tNFC_STATUS RW_T2tReadNDef(uint8_t *p_buffer, uint16_t buf_len) {
  ...
  if (!p_t2t->ndef_msg_len) {
    LOG(WARNING) << StringPrintf(
        "RW_T2tReadNDef - NDEF Message length is zero %u", p_t2t->ndef_msg_len);
