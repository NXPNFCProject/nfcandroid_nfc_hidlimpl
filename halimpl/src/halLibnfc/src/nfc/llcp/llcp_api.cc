...
void LLCP_SetTestParams(uint8_t version, uint16_t wks) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_SetTestParams () version:0x%02X, wks:0x%04X", version, wks);
...
void LLCP_GetDiscoveryConfig(uint8_t *p_wt, uint8_t *p_gen_bytes,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_GetDiscoveryConfig ()");
...
tLLCP_STATUS LLCP_ActivateLink(tLLCP_ACTIVATE_CONFIG config,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_ActivateLink () link_state = %d", llcp_cb.lcb.link_state);
...
tLLCP_STATUS LLCP_DeactivateLink(void) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_DeactivateLink () link_state = %d", llcp_cb.lcb.link_state);
...
uint8_t LLCP_RegisterServer(uint8_t reg_sap, uint8_t link_type,
  ...
  if (!p_app_cback) {
    LOG(ERROR) << StringPrintf(
        "LLCP_RegisterServer (): Callback must be provided");
  ...
  if (reg_sap == LLCP_INVALID_SAP) {
    ...
    LOG(ERROR) << StringPrintf(
        "LLCP_RegisterServer (): SAP (0x%x) is for link manager", reg_sap);
    return LLCP_INVALID_SAP;
  ...
  if (p_service_name.c_str()) {
    ...
    strncpy((char *)p_app_cb->p_service_name, p_service_name.c_str(),
            length + 1);
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_RegisterServer (): Registered SAP = 0x%02X", reg_sap);
...
uint8_t LLCP_RegisterClient(uint8_t link_type, tLLCP_APP_CBACK *p_app_cback) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_RegisterClient (): link_type = 0x%x", link_type);

  if (!p_app_cback) {
    LOG(ERROR) << StringPrintf(
        "LLCP_RegisterClient (): Callback must be provided");
    return LLCP_INVALID_SAP;
  } else if (((link_type & LLCP_LINK_TYPE_LOGICAL_DATA_LINK) == 0x00) &&
             ((link_type & LLCP_LINK_TYPE_DATA_LINK_CONNECTION) == 0x00)) {
    LOG(ERROR) << StringPrintf(
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_RegisterClient (): Registered SAP = 0x%02X", reg_sap);
...
tLLCP_STATUS LLCP_Deregister(uint8_t local_sap) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_Deregister () SAP:0x%x", local_sap);

  p_app_cb = llcp_util_get_app_cb(local_sap);

  if ((!p_app_cb) || (p_app_cb->p_app_cback == NULL)) {
    LOG(ERROR) << StringPrintf(
        "LLCP_Deregister (): SAP (0x%x) is not registered", local_sap);
    return LLCP_STATUS_FAIL;
  }
...
tLLCP_STATUS LLCP_SendUI(uint8_t ssap, uint8_t dsap, NFC_HDR *p_buf) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_SendUI () SSAP=0x%x, DSAP=0x%x", ssap, dsap);

  p_app_cb = llcp_util_get_app_cb(ssap);

  if ((p_app_cb == NULL) || (p_app_cb->p_app_cback == NULL)) {
    LOG(ERROR) << StringPrintf("LLCP_SendUI (): SSAP (0x%x) is not registered",
                               ssap);
    ...
    if (p_buf->len <= llcp_cb.lcb.peer_miu) {
      if (p_buf->offset >= LLCP_MIN_OFFSET) {
        ...
        LOG(ERROR) << StringPrintf(
            "LLCP_SendUI (): offset (%d) must be %d at least", p_buf->offset,
            LLCP_MIN_OFFSET);
      }
    } else {
      LOG(ERROR) << StringPrintf(
...
bool LLCP_ReadLogicalLinkData(uint8_t local_sap, uint32_t max_data_len,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_ReadLogicalLinkData () Local SAP:0x%x", local_sap);
  ...
  *p_data_len = 0;
  ...
  if ((p_app_cb) && (p_app_cb->p_app_cback)) {
    ...
    LOG(ERROR) << StringPrintf(
        "LLCP_ReadLogicalLinkData (): Unregistered SAP:0x%x", local_sap);
...
uint32_t LLCP_FlushLogicalLinkRxData(uint8_t local_sap) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_FlushLogicalLinkRxData () Local SAP:0x%x", local_sap);
  ...
  if ((p_app_cb) && (p_app_cb->p_app_cback)) {
    ...
    LOG(ERROR) << StringPrintf(
        "LLCP_FlushLogicalLinkRxData (): Unregistered SAP:0x%x", local_sap);
  }
...
tLLCP_STATUS LLCP_ConnectReq(uint8_t reg_sap, uint8_t dsap,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_ConnectReq () reg_sap=0x%x, DSAP=0x%x", reg_sap, dsap);
  ...
  if ((p_app_cb == NULL) || (p_app_cb->p_app_cback == NULL)) {
    LOG(ERROR) << StringPrintf(
        "LLCP_ConnectReq (): SSAP (0x%x) is not registered", reg_sap);
    return LLCP_STATUS_FAIL;
  }
...
tLLCP_STATUS LLCP_ConnectCfm(uint8_t local_sap, uint8_t remote_sap,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_ConnectCfm () Local SAP:0x%x, Remote SAP:0x%x)",
...
tLLCP_STATUS LLCP_SendData(uint8_t local_sap, uint8_t remote_sap,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_SendData () Local SAP:0x%x, Remote SAP:0x%x",
                      local_sap, remote_sap);
  ...
  if (p_dlcb) {
    if (p_dlcb->remote_miu >= p_buf->len) {
      if (p_buf->offset >= LLCP_MIN_OFFSET) {
        ...
        LOG(ERROR) << StringPrintf(
            "LLCP_SendData (): offset (%d) must be %d at least", p_buf->offset,
            LLCP_MIN_OFFSET);
      }
    } else {
      LOG(ERROR) << StringPrintf(
...
bool LLCP_ReadDataLinkData(uint8_t local_sap, uint8_t remote_sap,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_ReadDataLinkData () Local SAP:0x%x, Remote SAP:0x%x", local_sap,
      remote_sap);
  ...
  *p_data_len = 0;
  if (p_dlcb) {
    ...
    LOG(ERROR) << StringPrintf(
        "LLCP_ReadDataLinkData (): No data link connection");
...
uint32_t LLCP_FlushDataLinkRxData(uint8_t local_sap, uint8_t remote_sap) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_FlushDataLinkRxData () Local SAP:0x%x, Remote SAP:0x%x", local_sap,
      remote_sap);
  ...
  if (p_dlcb) {
    ...
    LOG(ERROR) << StringPrintf(
        "LLCP_FlushDataLinkRxData (): No data link connection");
  }
...
tLLCP_STATUS LLCP_SetTxCompleteNtf(uint8_t local_sap, uint8_t remote_sap) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "LLCP_SetTxCompleteNtf () Local SAP:0x%x, Remote SAP:0x%x", local_sap,
      remote_sap);
...
tLLCP_STATUS LLCP_SetLocalBusyStatus(uint8_t local_sap, uint8_t remote_sap,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_SetLocalBusyStatus () Local SAP:0x%x, is_busy=%d",
                      local_sap, is_busy);
...
uint16_t LLCP_GetRemoteWKS(void) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_GetRemoteWKS () WKS:0x%04x",
...
uint8_t LLCP_GetRemoteLSC(void) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_GetRemoteLSC () LSC:0x%x",
                      (llcp_cb.lcb.link_state == LLCP_LINK_STATE_ACTIVATED)
...
uint8_t LLCP_GetRemoteVersion(void) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_GetRemoteVersion () Version: 0x%x",
                      (llcp_cb.lcb.link_state == LLCP_LINK_STATE_ACTIVATED)
...
tLLCP_STATUS LLCP_DiscoverService(char *p_name, tLLCP_SDP_CBACK *p_cback,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("LLCP_DiscoverService () Service Name:%s", p_name);

  if (llcp_cb.lcb.link_state != LLCP_LINK_STATE_ACTIVATED) {
    LOG(ERROR) << StringPrintf(
        "LLCP_DiscoverService (): Link is not activated");
    return LLCP_STATUS_FAIL;
  }

  if (!p_cback) {
    LOG(ERROR) << StringPrintf(
        "LLCP_DiscoverService (): Callback must be provided.");
    return LLCP_STATUS_FAIL;
  }
  ...
  if ((llcp_cb.lcb.agreed_major_version == 0x01) &&
    ...
    LOG(ERROR) << StringPrintf(
        "LLCP_DiscoverService (): Peer doesn't support SNL");
    return LLCP_STATUS_FAIL;
  }
