...
void llcp_util_deallocate_data_link(tLLCP_DLCB *p_dlcb) {
  if (p_dlcb) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("llcp_util_deallocate_data_link (): local_sap = 0x%x",
                        p_dlcb->local_sap);
...
tLLCP_STATUS llcp_util_parse_connect(uint8_t *p_bytes, uint16_t length,
  ...
  while (length) {
    ...
    case LLCP_MIUX_TYPE:
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "llcp_util_parse_connect (): LLCP_MIUX_TYPE:%d", p_params->miu);
      break;
    ...
    case LLCP_RW_TYPE:
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "llcp_util_parse_connect (): LLCP_RW_TYPE:%d", p_params->rw);
      break;
    ...
    case LLCP_SN_TYPE:
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "llcp_util_parse_connect (): LLCP_SN_TYPE:<%s>", p_params->sn);
      break;

    default:
      LOG(ERROR) << StringPrintf(
          "llcp_util_parse_connect (): Unexpected type 0x%x", param_type);
      BE_STREAM_TO_UINT8(param_len, p);
      p += param_len;
      break;
    }
  ...
  *p_rw = LLCP_DEFAULT_RW;
  ...
  while (length) {
    ...
    case LLCP_MIUX_TYPE:
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("llcp_util_parse_cc (): LLCP_MIUX_TYPE:%d", *p_miu);
      break;
    ...
    case LLCP_RW_TYPE:
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("llcp_util_parse_cc (): LLCP_RW_TYPE:%d", *p_rw);
      break;
...
#if (NXP_EXTNS == TRUE)
  if (p_msg->len > p_cb->buff_size) {
    ((tNFC_EXT_HDR *)p_msg)->hdr_len =
        LLCP_PDU_HEADER_SIZE + LLCP_SEQUENCE_SIZE;
    p = ((tNFC_EXT_HDR *)p_msg)->hdr_info;
  } else {
    p = (uint8_t *)(p_msg + 1) + p_msg->offset;
  }
