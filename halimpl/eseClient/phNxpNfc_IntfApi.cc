...
NFCSTATUS phNxpNfc_InitLib() {
  ...
  theInstance.Initialize();  // start GKI, NCI task, NFC task
  {
    ...
    if (stat != NFA_STATUS_OK) {
      ALOGE("%s: fail ee register; error=0x%X", fn,
                              stat);
      return false;
    }
    if (SEM_WAIT(cb_data)) {
...
NFCSTATUS phNxpNfc_DeInitLib() {
  ALOGE("phNxpNfc_DeInitLib enter");
  tNFA_STATUS stat = NFA_STATUS_FAILED;
    ...
    if (stat == NFA_STATUS_OK) {
      ...
      ALOGE("%s: fail disable; error=0x%X", __func__,
                               stat);
    }
  }
  isNfcInitialzed = false;
  ALOGE("phNxpNfc_DeInitLib exit");
  return stat;
}
