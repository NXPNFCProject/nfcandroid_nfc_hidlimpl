...
EseClient EseClient::sEseClientInstance;
EseClient::EseClient() {}

EseClient &EseClient::getInstance() { return sEseClientInstance; }
...
bool SE_Close(__attribute__((unused)) int16_t mHandle)
{
    return true;
    /*if(mHandle != 0)
    {
...
{
  return SeChannelCallback::INTF_NFC;
}
/***************************************************************************
**
** Function:        EseClient::checkEseClientUpdate
**
...
**
*******************************************************************************/
void EseClient::checkEseClientUpdate()
{
  ALOGD("%s enter:  ", __func__);
  bool nfcSEIntfPresent = false;
  char nfcterminal[5];
  se_intf = jcDnld.checkEseUpdateRequired(ESE_INTF_NFC);
...
/***************************************************************************
**
** Function:        EseClient::perform_eSEClientUpdate
**
...
**
*******************************************************************************/
SESTATUS EseClient::perform_eSEClientUpdate() {
...
void* eSEClientUpdate_ThreadHandler(void* data) {
  ...
  if(mHalNxpEse != nullptr)
  {
    ...
    mHalNxpEse->ioctl(HAL_ESE_IOCTL_GET_ESE_UPDATE_STATE, data, IoctlCallback_DwpClient);
