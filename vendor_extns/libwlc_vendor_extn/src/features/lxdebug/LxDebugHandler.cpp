...
WLCSTATUS LxDebugHandler::handleVendorNciMessage(uint16_t dataLen,
                                                 const uint8_t *pData) {
  ...
  if (subGid == LXDEBUG_SUB_GID) {
    ...
    default:
      ...
      status = WLCSTATUS_EXTN_FEATURE_FAILURE;
      break;
    }
...
WLCSTATUS LxDebugHandler::handleVendorNciRspNtf(uint16_t dataLen,
                                                uint8_t *pData) {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                 "LxDebugHandler::%s "
                 "Enter dataLen:%d",
...
WLCSTATUS LxDebugHandler::processExtnWrite(uint16_t *dataLen, uint8_t *pData) {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "LxDebugHandler %s Enter ",
                 __func__);
  ...
  if (mLxDebugMngr->isEFDMStarted()) {
    ...
    if (mLxDebugMngr->processRfDiscCmd(rfDiscCmd) == WLCSTATUS_EXTN_FEATURE_SUCCESS) {
      if (rfDiscCmd.size() <= NCI_MAX_DATA_LEN) {
        ...
        *dataLen = rfDiscCmd.size();
        ...
        return WLCSTATUS_EXTN_FEATURE_SUCCESS_CONTINUE;
      } else {
