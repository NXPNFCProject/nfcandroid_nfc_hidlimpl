...
#ifndef WLC_EXTENSION_WRITER_H
#define WLC_EXTENSION_WRITER_H
...
/** \addtogroup WLC_EXTENSION_WRITER_API_INTERFACE
 *  @brief  interface to perform the exclusive and non exclusive write to
 * controller.
 *  @{
 */
class WlcExtensionWriter {
public:
  WlcExtensionWriter(const WlcExtensionWriter &) = delete;
  WlcExtensionWriter &operator=(const WlcExtensionWriter &) = delete;
  /**
   * @brief Get the singleton of this object.
   * @return Reference to this object.
   *
   */
  static WlcExtensionWriter *getInstance();
  /**
   ...
   *
   */
  WLCSTATUS write(const uint8_t *pBuffer, uint16_t wLength,
  ...
  static inline void finalize() {
    if (sWlcExtensionWriter != nullptr) {
      delete (sWlcExtensionWriter);
      sWlcExtensionWriter = nullptr;
    }
  }

std::vector<uint8_t> lastNciCmd;
private:
  static WlcExtensionWriter *sWlcExtensionWriter;
  WlcExtensionWriter();
  ~WlcExtensionWriter();
