...
public:
  ...
  virtual WLCSTATUS handleVendorNciMessage(uint16_t dataLen,
   ...
   *
   */
  virtual WLCSTATUS handleVendorNciRspNtf(uint16_t dataLen, uint8_t *pData) = 0;
   ...
   *
   */
  virtual WLCSTATUS handleHalEvent(uint8_t errorCode) {
   ...
   * WLCSTATUS_EXTN_FEATURE_FAILURE.
   */
  virtual WLCSTATUS processExtnWrite(uint16_t *dataLen, uint8_t *pData) {
    NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "IEventHandler::%s Enter",
                   __func__);
    return WLCSTATUS_EXTN_FEATURE_FAILURE;
  }
