...
using std::vector;
std::unique_ptr<NfcFirmwareInfo> NfcFirmwareInfo::instance = nullptr;
...
NfcFirmwareInfo *NfcFirmwareInfo::getInstance() {
  if (!instance) {
    instance = std::unique_ptr<NfcFirmwareInfo>(new NfcFirmwareInfo());
  }
  return instance.get();
}
...
void NfcFirmwareInfo::fetchAndSendFwVersionNtf() {
  ...
  const uint8_t FwVerNciVendorRspLen = FW_VER_NCI_VENDOR_RSP.size();
