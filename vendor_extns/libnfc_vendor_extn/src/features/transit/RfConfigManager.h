...
public:
  ...
  NFCSTATUS processRsp(std::vector<uint8_t> rfConfigRsp);

  uint8_t sendNciCmd(std::vector<uint8_t> rfConfigCmd);

  bool checkUpdateRfRegisterConfig(std::vector<uint8_t> rfConfigCmd);

  /**
   * @brief Get the Update Prop Rf Set Config object
   *
   * @param newValue To be update at the index position
   * @param propCmdResData Update the prop response buffer based on
   *                 prop getConfig response.
   * @return true/false
   */
  bool isRFConfigUpdateRequired(unsigned newValue,
                                std::vector<uint8_t> &propCmdResData);

  /**
   * @brief Update the set RF settings.
   *
   * @param setConfCmd Update the set config buffer based on getConfig
   * @param getResData Response data.
   * @return true/false
   */
  bool updateRfSetConfig(std::vector<uint8_t> &setConfCmd,
                         std::vector<uint8_t> &getResData);
  ...
  bool isUpdateThreshold(uint8_t index, unsigned &new_value,
                         std::vector<uint8_t> &lpdet_cmd_response);
   ...
   */
  bool sendCommandAndValidateResponse(std::vector<uint8_t> cmd_get_rfconfval);
...
private:
  ...
  static std::unique_ptr<RfConfigManager> instance;

  RfConfigManager();
  ~RfConfigManager();

  friend struct std::default_delete<RfConfigManager>;

  /* Below are A085 RF register bitPositions */
  ...
  /**
   * @brief To add support to update new RF Config at runtime
   *        1. Append RF_Config in tSetDynamicRfConfigType in
   *           UPDATE_<RF_Config> format
   *        2. Add mapping in tokenMap.
   *        3. Add handling in setConfig() API
   */
