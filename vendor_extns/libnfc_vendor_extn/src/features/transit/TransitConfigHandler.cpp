...
std::unique_ptr<TransitConfigHandler> TransitConfigHandler::instance = nullptr;
...
TransitConfigHandler *TransitConfigHandler::getInstance() {
  if (!instance) {
    instance =
        std::unique_ptr<TransitConfigHandler>(new TransitConfigHandler());
  }
  return instance.get();
}
...
bool TransitConfigHandler::updateVendorConfig(vector<uint8_t> configPkt) {
  ...
  if (configPkt.size() > 0) {
    const std::string configStr(configPkt.begin(), configPkt.end());
