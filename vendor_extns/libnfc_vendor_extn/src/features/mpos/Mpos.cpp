...
Mpos::~Mpos() {
  ...
  tagOperationTimer.kill(&tagOperationTimerId);
  tagRemovalTimer.kill(&tagRemovalTimerId);
  startStopGuardTimer.kill(&startStopGuardTimerId);
}

Mpos *Mpos::getInstance() {
  if (!instance) {
    instance = std::unique_ptr<Mpos>(new Mpos());
  }
  return instance.get();
}
...
static bool isValidNciConfig(const uint8_t *buffer, size_t length) {
  ...
  const size_t expectedLength = sizeof(selectProfile);
...
static void sendSelectProfileCmd() {
  ...
  if (isFound && isValidNciConfig(selProfileConfig, bufflen)) {
    selProfileConfig[bufflen - 1] = 0x01;
    Mpos::getInstance()->updateState(
        MPOS_STATE_WAIT_FOR_PROFILE_SELECT_RESPONSE);
    NFC_WRITE(selProfileConfig, bufflen);
  } else {
...
static void notifyRfDiscoveryStopped() {
  ...
  Mpos::getInstance()->tagOperationTimer.kill(
      &Mpos::getInstance()->tagOperationTimerId);
  Mpos::getInstance()->mRecoveryTimer.kill(
      &Mpos::getInstance()->mRecoveryTimerId);
...
static void sendDeselectProfileCmd() {
  ...
  if (isFound && isValidNciConfig(selProfileConfig, bufflen)) {
    selProfileConfig[bufflen - 1] = 0x00;
    Mpos::getInstance()->updateState(
...
NFCSTATUS Mpos::processMposNciRspNtf(const std::vector<uint8_t> &pData) {
  const ScrState_t state = getScrState(pData);
...
bool Mpos::isExternalFieldDetected(const std::vector<uint8_t> &pData) {
  ...
  while (numOfParams > 0) {
    // Parse all TLVS
    const uint8_t tag = pData[index++];
    const uint8_t len = pData[index++];
    std::vector<uint8_t> value((pData.data() + index),
                               (pData.data() + index + len));
    index += len;
    numOfParams--;
    if (tag == static_cast<uint8_t>(NfcCorePropRFMgmtSysGenInfoTags::EMVCO_PCD) &&
        len == 0x01 &&
        value[0] == static_cast<uint8_t>(NfcCorePropRfMgmtSysGenInfoValues::
...
void Mpos::recoveryTimeoutHandler(union sigval val) {
  ...
  if (getInstance()->mState == MPOS_STATE_RECOVERY_ON_EXT_FIELD_DETECT) {
    ...
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s: Invalid state returning",
                   __func__);
  }
  return;
}
...
bool Mpos::isSeReaderRestarted(const std::vector<uint8_t> &pData) {
  ...
  const uint8_t aid = pData[5];
  const uint8_t aidLength = pData[6];
  ...
  const uint8_t paramType = pData[7 + aidLength];
  const uint8_t paramLength = pData[8 + aidLength];
  ...
  const uint8_t Event = pData[9 + aidLength];
  const uint8_t Version = pData[10 + aidLength];
  const uint8_t Code = pData[11 + aidLength];
