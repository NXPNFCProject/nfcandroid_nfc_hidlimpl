...
std::unique_ptr<Srd> Srd::instance = nullptr;
...
Srd *Srd::getInstance() {
  if (!instance) {
    instance = std::unique_ptr<Srd>(new Srd());
  }
  return instance.get();
}
...
/*Initialize the SRD module*/
void Srd::Initialize() {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s Enter", __func__);
  const uint16_t SRD_DISABLE_MASK = 0xFFFF;
  ...
  isFound = GetNxpByteArrayValue(
      NAME_NXP_SRD_TIMEOUT,
      static_cast<char *>(reinterpret_cast<void *>(srd_config.get())), bufflen,
      &retlen);
  if ((isFound)) {
...
NFCSTATUS Srd::processSrdNciRspNtf(std::vector<uint8_t> &pRspBuffer) {
  ...
  case SRD_STATE_W4_HCI_CONNECTIVITY_EVT: {
    const SrdEvent_t srdEvent = checkSrdEvent(pRspBuffer);
    if (srdEvent == SRD_UNKNOWN_EVT) {
...
Srd::processDefaultDiscMapRsp(const std::vector<uint8_t> &pRspBuffer) {
  ...
  for (const int value : pRspBuffer) {
  ...
  if (mGidOid == NCI_DISC_MAP_RSP_GID_OID) {
    ...
    switchToDefaultHandler();
    for (const int value : mDiscStopResp) {
      NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s: enter %x", __func__, value);
    }
...
NFCSTATUS Srd::processPwrLinkRsp(const std::vector<uint8_t> &pRspBuffer) {
  ...
  if (mCmdBuffer.size() >= NCI_PAYLOAD_LEN_INDEX &&
    ...
    const uint8_t cmdGid = (mCmdBuffer[NCI_GID_INDEX] & EXT_NCI_GID_MASK);
    const uint8_t cmdOid = (mCmdBuffer[NCI_OID_INDEX] & EXT_NCI_OID_MASK);
...
SrdEvent_t Srd::checkSrdEvent(const std::vector<uint8_t> &pDataPacket) {
  if (pDataPacket.size() >= NCI_PAYLOAD_LEN_INDEX) {
    const uint8_t dataGid = (pDataPacket[NCI_GID_INDEX] & EXT_NCI_GID_MASK);
    const uint8_t dataOid = (pDataPacket[NCI_OID_INDEX] & EXT_NCI_OID_MASK);
...
Srd::parseAndCheckSrdEvent(const std::vector<uint8_t> &pRspHciEvtData) {
  ...
  if (((event & SRD_HCI_EVT_TRANSACTION_MASK) == SRD_HCI_EVT_TRANSACTION) &&
    ...
    NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s: HCI Transaction event",
                   __func__);
...
void Srd::processDiscoverMapRsp(const std::vector<uint8_t> &pRspHciEvtData) {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s: enter", __func__);
  const uint16_t mGidOid = ((pRspHciEvtData[0] << 8) | pRspHciEvtData[1]);
  switch (mGidOid) {
...
bool Srd::checkAndHandleRfDiscNtf(const std::vector<uint8_t> &pRspBuffer) {
  if (pRspBuffer.size() > NCI_RF_DISC_NTF_PROTOCOL_INDEX) {
    const uint16_t gidOid = ((pRspBuffer[0] << 8) | pRspBuffer[1]);
...
NFCSTATUS Srd::processExtnsWrite(uint16_t *dataLen, uint8_t *pData) {
  ...
  const uint8_t msgType = pData[NCI_MSG_TYPE_INDEX] >> 5;
  switch (msgType) {
