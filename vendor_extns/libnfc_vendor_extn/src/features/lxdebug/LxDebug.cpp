...
using std::vector;
std::unique_ptr<LxDebug> LxDebug::instance = nullptr;

LxDebug *LxDebug::getInstance() {
  if (!instance) {
    instance = std::unique_ptr<LxDebug>(new LxDebug());
  }
  return instance.get();
}
...
EfdmStatus_t LxDebug::isFieldDetectEnabledFromConfig() {
  ...
  if (PlatformAbstractionLayer::getInstance()->palGetNxpNumValue(
          NAME_NXP_EXTENDED_FIELD_DETECT_MODE, &extended_field_mode,
...
NFCSTATUS LxDebug::processRfDiscCmd(vector<uint8_t> &rfDiscCmd) {
  const uint8_t NCI_EFDM_PAYLOAD_LEN = 2;
  const uint8_t NCI_RF_DISC_PAYLOAD_LEN_INDEX = 2;
  const uint8_t NCI_RF_DISC_NUM_OF_CONFIG_INDEX = 3;
  NXPLOG_EXTNS_I(NXPLOG_ITEM_NXP_GEN_EXTN, "LxDebug %s Entry ", __func__);

  if (rfDiscCmd.size() > 1) {
    const uint16_t gidOid = ((rfDiscCmd[0] << 8) | rfDiscCmd[1]);
