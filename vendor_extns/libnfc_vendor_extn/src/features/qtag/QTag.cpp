...
using std::vector;
std::unique_ptr<QTag> QTag::instance = nullptr;
...
QTag *QTag::getInstance() {
  if (!instance) {
    instance = std::unique_ptr<QTag>(new QTag());
  }
  return instance.get();
}
...
bool QTag::isQTagRfIntfNtf(vector<uint8_t> rfIntfNtf) {
  const uint16_t mGidOid = ((rfIntfNtf[0] << 8) | rfIntfNtf[1]);
