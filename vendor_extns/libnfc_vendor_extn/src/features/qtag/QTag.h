...
public:
   ...
   */
  bool isQTagRfIntfNtf(std::vector<uint8_t> rfIntfNtf);
   ...
   */
  NFCSTATUS processIntActivatedNtf(std::vector<uint8_t> rfIntfNtf);
   ...
   */
  bool checkDiscCmd(std::vector<uint8_t> rfDiscCmd);
   ...
   */
  bool isObserveModeEnabled(std::vector<uint8_t> rfDiscCmd);
...
private:
  static std::unique_ptr<QTag> instance;

  QTag();
  ~QTag();
  friend struct std::default_delete<QTag>;
