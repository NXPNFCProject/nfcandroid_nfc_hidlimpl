...
**
*******************************************************************************/
intptr_t MessageQueue_get(key_t key, int msgFlag) {
  phDal4Nfc_message_queue_t *pQueue;
  UNUSED_PROP(key);
  UNUSED_PROP(msgFlag);
  pQueue =
      static_cast<phDal4Nfc_message_queue_t *>(malloc(sizeof(phDal4Nfc_message_queue_t)));
  if (pQueue == NULL)
    return -1;
...
/*******************************************************************************
**
** Function         MessageQueue_postSemaphore
**
** Description      Unblocks thread waiting on this message queue
**
** Parameters       msgId - message queue handle
**
** Returns          None
**
*******************************************************************************/
void MessageQueue_postSemaphore(intptr_t msgId) {
  phDal4Nfc_message_queue_t *pQueue = (phDal4Nfc_message_queue_t *)msgId;
  if (pQueue != NULL) {
...
/*******************************************************************************
**
** Function         MessageQueue_destroy
**
** Description      Destroys msg queue. After this api Msg Q will not be usable.
**
** Parameters       msgId - message queue handle
**
** Returns          None
**
*******************************************************************************/
void MessageQueue_destroy(intptr_t msgId) {
  phDal4Nfc_message_queue_t *pQueue = (phDal4Nfc_message_queue_t *)msgId;

  if (pQueue != NULL) {
    if (sem_destroy(&pQueue->nProcessSemaphore)) {
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "Failed to destroy semaphore (errno=0x%08x)", errno);
    }
...
**
*******************************************************************************/
intptr_t MessageQueue_send(intptr_t msgId, phLibNfc_Message_t *msg,
                           int msgFlag) {
  phDal4Nfc_message_queue_t *pQueue;
  ...
  pQueue = (phDal4Nfc_message_queue_t *)msgId;
  pNew = (phDal4Nfc_message_queue_item_t *)malloc(
      sizeof(phDal4Nfc_message_queue_item_t));
  if (pNew == NULL) {
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "Failed to malloc pNew errno = %d",
                   errno);
    return -1;
  }
...
**
*******************************************************************************/
int MessageQueue_receive(intptr_t msgId, phLibNfc_Message_t *msg, long msgType,
                         int msgFlag) {
  phDal4Nfc_message_queue_t *pQueue;
  phDal4Nfc_message_queue_item_t *p;
  UNUSED_PROP(msgFlag);
  ...
  if (-1 == sem_wait(&pQueue->nProcessSemaphore)) {
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                   "sem_wait didn't return success errno = %d", errno);
  }
