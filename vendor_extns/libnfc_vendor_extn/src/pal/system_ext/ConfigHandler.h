...
class ConfigHandler {
 public:
   ...
   *        and adds/replaces the config and its value
   * @param pConfigMap config map pointer received from libNfc
   * @return None
   *
   */
  void updateConfig(std::map<std::string, ConfigValue> *pConfigMap);
  /**
  ...
  static inline void finalize() {
    if (sConfigHandler != nullptr) {
      sConfigHandler.reset();
    }
  }

 private:
   static std::unique_ptr<ConfigHandler> sConfigHandler; // singleton object
   /**
    * @brief Route location values defined by OEM
    *
    */
   enum class OemRouteLoc : uint8_t {
     HOST = 0x00,
     ESE = 0x01,
     UICC = 0x02,
     UICC2 = 0x03,
    ...
    *
    */
   enum class AospRouteLoc : uint8_t {
     HOST = 0x00,
     ESE = 0xC0,
     UICC = 0x80,
     UICC2 = 0x81,
    ...
    *
    */
   std::map<std::string, ConfigValue> *mpConfigMap;
   ...
   friend struct std::default_delete<ConfigHandler>;
   /**
    ...
    *
    */
   void adaptLegacySameKeyType(std::string oemKey, std::string aospKey,
                               std::map<std::string, ConfigValue> nxpNfcConfig);
    ...
    *
    */
   void adaptLegacyDiffKeyType(std::vector<std::string> oemKey,
                               std::string aospKey,
                               std::map<std::string, ConfigValue> nxpNfcConfig);
    ...
    */
   void
   adaptLegacyConfigs(const std::map<std::string, ConfigValue> &nxpNfcConfig);
    ...
    *
    */
   void mapRouteConfig(std::map<std::string, ConfigValue> nxpNfcConfig);
    ...
    *
    */
   std::map<std::string, ConfigValue> parseFromString(const std::string &input);
    ...
    */
   std::map<std::string, ConfigValue> loadNfcNxpConfig();
    ...
    *
    */
   std::map<std::string, std::string> getRouteConfigNameMap();
    ...
    *
    */
   std::map<OemRouteLoc, AospRouteLoc> getRouteConfigValueMap();
    ...
    *
    */
   std::vector<uint8_t> getMappedRouteValue(std::vector<uint8_t> oemRouteVals);
    ...
    *
    */
   void mapUnsignedRouteConfig(unsigned oemRouteVal, std::string aospRouteName);
    ...
    *
    */
   void mapByteRouteConfig(std::vector<uint8_t> oemRouteVals,
                           std::string aospRouteName);
    ...
    *
    */
   std::string getConfigValueStr(ConfigValue configValue);
};
/** @}*/
