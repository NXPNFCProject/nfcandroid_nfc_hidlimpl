...
std::unique_ptr<PlatformAbstractionLayer>
    PlatformAbstractionLayer::sPlatformAbstractionLayer = nullptr;
string propVal;
...
PlatformAbstractionLayer *PlatformAbstractionLayer::getInstance() {
  if (!sPlatformAbstractionLayer) {
    sPlatformAbstractionLayer = std::unique_ptr<PlatformAbstractionLayer>(
        new PlatformAbstractionLayer());
  }
  return sPlatformAbstractionLayer.get();
}
...
uint32_t PlatformAbstractionLayer::palGetLibNciConfig(const char* key, bool& found) {
  ...
  if (it != libNfcNciConfigMap.end()) {
      ...
      const uint32_t defaultValue = it->second.second;  // Get default value from table
