...
ConfigHandler* ConfigHandler::getInstance() {
  if (sConfigHandler == nullptr) {
    sConfigHandler = std::unique_ptr<ConfigHandler>(new ConfigHandler());
  }
  return sConfigHandler.get();
}
...
map<string, ConfigValue> ConfigHandler::loadNfcNxpConfig() {
  const string confString = PlatformAbstractionLayer::getInstance()->getVendorParam(
      "libnfc-nxp.conf");
  map<string, ConfigValue> configs;
  if (!confString.empty()) {
...
string ConfigHandler::getConfigValueStr(ConfigValue value) {
  ...
  if (value.getType() == ConfigValue::UNSIGNED) {
    ...
    const vector<uint8_t> values = value.getBytes();
    const size_t bufferSize = values.size() * 5 + 1;
...
void ConfigHandler::adaptLegacyConfigs(const map<string, ConfigValue>& nxpNfcConfig) {
  const map<vector<string>, string> updateConfigMap{
  ...
  for (auto const& [oemRouteName, aospRouteName] : routeNameMap) {
    ...
    if (oemRoutePair == mpConfigMap->end()) {
      ...
      if (oemRoutePair == nxpNfcConfig.end()) {
        NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                       "%s Route Conf '%s' not found!", __func__,
                       oemRouteName.c_str());
        continue;
      }
    }
    if (oemRoutePair->second.getType() == ConfigValue::UNSIGNED) {
      ...
      NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "%s Unknown Route value type '%d' for  %s!", __func__,
