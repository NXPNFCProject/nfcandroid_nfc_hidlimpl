...
bool PalIntervalTimer::set(unsigned long ms, void *ptr, TIMER_FUNC cb, timer_t *timerId,
                           bool oneShot) {
  ...
  if (oneShot) {
    // No repeat, timer will be triggered only once
    ts.it_interval.tv_sec = 0;
    ts.it_interval.tv_nsec = 0;
  } else {
    // Timer will be triggered repeatedly at specified interval
