...
public:
  ...
  /**
   * @brief Add the event handler and its type to handler's list of controller.
   * @return void
   *
   */
  ...
  static inline void finalize() {
    if (sNfcExtensionController != nullptr) {
      sNfcExtensionController.reset();
    }
  }

private:
  static std::unique_ptr<NfcExtensionController>
      sNfcExtensionController; // singleton object
   ...
   */
  ~NfcExtensionController();
  // in order to keep destructor as private, we need to have friend struct
  NfcExtensionController(const NfcExtensionController &) = delete;
  NfcExtensionController &operator=(const NfcExtensionController &) = delete;
  friend struct std::default_delete<NfcExtensionController>;
  /**
