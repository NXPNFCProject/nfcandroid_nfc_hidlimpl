...
std::unique_ptr<NciCommandBuilder> NciCommandBuilder::sNciCommandBuilder =
    nullptr;
...
NciCommandBuilder *NciCommandBuilder::getInstance() {
  if (!sNciCommandBuilder) {
    sNciCommandBuilder =
        std::unique_ptr<NciCommandBuilder>(new NciCommandBuilder());
  }
  return sNciCommandBuilder.get();
}
