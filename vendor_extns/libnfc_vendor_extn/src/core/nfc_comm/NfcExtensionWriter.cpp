...
std::unique_ptr<NfcExtensionWriter> NfcExtensionWriter::sNfcExtensionWriter =
    nullptr;
...
NfcExtensionWriter *NfcExtensionWriter::getInstance() {
  if (!sNfcExtensionWriter) {
    sNfcExtensionWriter =
        std::unique_ptr<NfcExtensionWriter>(new NfcExtensionWriter());
  }
  return sNfcExtensionWriter.get();
}
...
NFCSTATUS NfcExtensionWriter::write(const uint8_t *pBuffer, uint16_t wLength,
  ...
  if (mWriteRspTimer.set(timeout, static_cast<void*>(this), writeRspTimeoutCbk,
                         &mWriteRspTimerId)) {
    cmdData.clear();
...
void NfcExtensionWriter::stopWriteRspTimer(const uint8_t *pRspBuffer,
  ...
  if (cmdData.size() >= NCI_PAYLOAD_LEN_INDEX &&
    ...
    const uint8_t cmdGid = (cmdData[NCI_GID_INDEX] & EXT_NCI_GID_MASK);
    const uint8_t cmdOid = (cmdData[NCI_OID_INDEX] & EXT_NCI_OID_MASK);
