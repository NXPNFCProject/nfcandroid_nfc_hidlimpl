...
NFCSTATUS phNxpNciHal_PowerTrackerInit(PowerTrackerHandle* outHandle) {
  ...
  // Open power_tracker_v2 shared library
  NXPLOG_NCIHAL_D("Opening (/vendor/lib64/power_tracker_v2.so)");
  outHandle->dlHandle = dlopen("/vendor/lib64/power_tracker_v2.so", RTLD_NOW);
  if (outHandle->dlHandle == NULL) {
    NXPLOG_NCIHAL_D("Error : opening (/vendor/lib64/power_tracker_v2.so) %s!!",
                    dlerror());
    return NFCSTATUS_FAILED;
  }
  ...
  NXPLOG_NCIHAL_D("Opened (/vendor/lib64/power_tracker_v2.so)");
  return status;
}
...
NFCSTATUS phNxpNciHal_PowerTrackerDeinit(PowerTrackerHandle* outHandle) {
  NXPLOG_NCIHAL_D("Closing (/vendor/lib64/power_tracker_v2.so)");
  if (outHandle == NULL) {
