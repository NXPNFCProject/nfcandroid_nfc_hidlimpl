...
NFCSTATUS DualAntenna::handleVendorNciMessage(uint16_t dataLen,
  ...
  if ((dataLen < CMD_MIN_DATA_LENGTH) || (pData[NCI_OID_INDEX] != NCI_ROW_PROP_OID_VAL) ||
  ...
  switch (pData[DUAL_ANTENNA_SUB_GID_OID_INDEX]) {
    case DUAL_ANTENNA_IS_SUPPORTED: {
      uint8_t num = 0;
      status = ((!(GetNxpNumValue(NAME_NXP_DUAL_ANTENNA_SUPPORTED, &num,
                                  sizeof(num)))))
                   ? NFCSTATUS_FAILED
                   : NFCSTATUS_SUCCESS;
      break;
    ...
    case DUAL_ANTENNA_SET_DISCOVERY: {
      ...
      if ((mDualAntennaContext.mAntOneConfig ==
               DUAL_ANTENNA_NFC_PASSIVE_ABFKQ ||
  ...
  phNxpHal_NfcDataCallback(sizeof(DUAL_ANTENNA_STATUS_RSP), DUAL_ANTENNA_STATUS_RSP);
...
NFCSTATUS DualAntenna::handleVendorNciRspNtf(uint16_t dataLen, uint8_t* pData) {
  ...
  if ((dataLen < CMD_MIN_DATA_LENGTH) || (mDualAntennaContext.mDualAntennaRequest != true) ||
...
NFCSTATUS DualAntenna::applyDualAntennaSettings(bool feature) {
  ...
  return phNxpHal_EnqueueWrite(cmd.data(), cmd.size());
}
...
NFCSTATUS DualAntenna::setDiscoveryTechnology_DualAntenna() {
  ...
  cmd_dual_antenna_set_discovery[ANT_TWO_CONFIG_INDEX] =
      mDualAntennaContext.mAntTwoConfig;
  return phNxpHal_EnqueueWrite(cmd_dual_antenna_set_discovery.data(),
...
bool DualAntenna::isDualAntennaSupported() {
  NFCSTATUS status = NFCSTATUS_FAILED;
  uint8_t num = 0;
  if (!GetNxpNumValue(NAME_NXP_DUAL_ANTENNA_SUPPORTED, &num, sizeof(num)))
    return false;
  ...
  if (IDLE == phNxpExtn_NfcGetRfState()) {
    ...
    status = phNxpHal_EnqueueWrite(DISABLE_DISC_CMD.data(), DISABLE_DISC_CMD.size());
  }
...
NFCSTATUS DualAntenna::setDualAntennaPollMode() {
  ...
  vector<uint8_t> cmd_dual_antenna_set_polling = {0x20, 0x02, 0x04, 0x01,
                                                  0x02, 0X01, 0x01};
  if (!(GetNxpNumValue(NAME_NXP_DUAL_ANTENNA_FOLDABLE, &num, sizeof(num))))
    return NFCSTATUS_FAILED;
  if (num != 0x00 && (num | DUAL_ANTENNA_FOLDABLE) == DUAL_ANTENNA_FOLDABLE) {
    cmd_dual_antenna_set_polling[READER_MODE_CONFIG_INDEX] =
      mDualAntennaContext.mConfigReaderMode;
    return phNxpHal_EnqueueWrite(cmd_dual_antenna_set_polling.data(),
...
NFCSTATUS DualAntenna::sendRfDeactivate(const uint8_t* pData) {
  ...
  vector<uint8_t> DISABLE_DISC_CMD = {0x21, 0x06, 0x01, 0x00};
  return phNxpHal_EnqueueWrite(DISABLE_DISC_CMD.data(),
                               DISABLE_DISC_CMD.size());
}
...
NFCSTATUS DualAntenna::sendRfDiscCmd() {
  ...
  if (mDualAntennaContext.mAntOneConfig != 0x00 &&
    ...
    if ((mDualAntennaContext.mAntOneConfig &
         mDualAntennaContext.mAntTwoConfig) == DUAL_ANTENNA_NFC_PASSIVE_Q) {
      return phNxpHal_EnqueueWrite(RF_DISC_CMD_ONLY_QTAG.data(),
                                   RF_DISC_CMD_ONLY_QTAG.size());
    }
  } else {
    if ((mDualAntennaContext.mAntOneConfig |
         mDualAntennaContext.mAntTwoConfig) == DUAL_ANTENNA_NFC_PASSIVE_Q) {
  ...
  return phNxpHal_EnqueueWrite(RF_DISC_CMD.data(), RF_DISC_CMD.size());
}
...
NFCSTATUS DualAntenna::sendConDiscParamCmd() {
  ...
  cmd_con_disc_param[READER_MODE_CONFIG_INDEX] =
      mDualAntennaContext.mConfigReaderMode;
  status = phNxpHal_EnqueueWrite(cmd_con_disc_param.data(),
                                 cmd_con_disc_param.size());
