...
extern NFCSTATUS phNxpNciHal_releasePendingRead();
...
static __attribute__((constructor)) void onLoadLibrary(void) {
  NXPLOG_NCIHAL_D("%s:power tracker not available", __func__);
}
...
NFCSTATUS phNxpNciHal_fw_download(uint8_t seq_handler_offset,
  ...
  if (!bIsNfccDlState) {
    ...
    NXPLOG_NCIHAL_D("nfcFL.nfccFL.chipType %s\n",
                    pConfigFL->product[nfcFL.chipType]);

    if (IS_CHIP_TYPE_GE(sn100u) && IS_CHIP_TYPE_NE(ctn820)) {
  ...
  if (NFCSTATUS_SUCCESS == status) {
    ...
    if (IS_CHIP_TYPE_GE(sn400u)) {
      NXPLOG_NCIHAL_D("going for v2 FW Download");
      phTmlNfc_ReadAbort();
      phNxpNciHal_releasePendingRead();
      // DnldV2BuilderInstance.e
      // phTmlNfc_IoCtl(phTmlNfc_e_ResetDevice);
      status = DnldV2BuilderInstance.doFwDnld();
    } else {
      NXPLOG_NCIHAL_D("going for regular FW Download");
...
NFCSTATUS phNxpNciHal_CheckValidFwVersion(void) {
  ...
  } else if ((fw_maj_ver == FW_MOBILE_MAJOR_NUMBER_SN400A) ||
             (fw_maj_ver == FW_MOBILE_MAJOR_NUMBER_CTN820)) {
    status = NFCSTATUS_SUCCESS;
  } else {
  ...
  if (NFCSTATUS_SUCCESS == phNxpNciHal_nfcc_core_reset_init(true)) {
    ...
    if (bVenResetRequired) {
      if (NFCSTATUS_SUCCESS == phNxpNciHal_getChipInfoInFwDnldMode(true))
        bIsNfccDlState = true;
      /* Add preconditions for Tearing mode and Degraded LUNA FW dnld*/
      else {
        /* SN400 detected */
        phTmlNfc_ReadAbort();
        phTmlNfc_EnableFwDnldMode(true);
        /* Set the obtained device handle to download module */
        phDnldNfc_SetHwDevHandle();
        if (NFCSTATUS_SUCCESS == DnldV2BuilderInstance.getInfo()) {
          DnldV2BuilderInstance.setChipDetected(true);
          bIsNfccDlState = true;
        }
      }
    }
...
int phNxpNciHal_open(nfc_stack_callback_t* p_cback,
  ...
  NFCSTATUS status = NFCSTATUS_SUCCESS;
  NXPLOG_NCIHAL_E("phNxpNciHal_open WLC HAL OPEN");
...
int phNxpNciHal_fw_mw_ver_check() {
  ...
  switch (nfcFL.chipType) {
    ...
    case sn400u:
      if ((rom_version ==
           (FW_MOBILE_ROM_VERSION_SN400A | FW_MOBILE_ROM_VERSION_SN400B)) &&
          (fw_maj_ver ==
           (FW_MOBILE_MAJOR_NUMBER_SN400A | FW_MOBILE_MAJOR_NUMBER_SN400B)))
        status = NFCSTATUS_SUCCESS;
      break;
    case ctn820:
      if ((rom_version == FW_MOBILE_ROM_VERSION_CTN820) &&
          (fw_maj_ver == FW_MOBILE_MAJOR_NUMBER_CTN820))
        status = NFCSTATUS_SUCCESS;
      break;
    default:
...
int phNxpNciHal_core_initialized(uint16_t core_init_rsp_params_len,
  ...
  if (IS_CHIP_TYPE_EQ(ctn820)) {
    uint8_t configPMU_VDDPA_FINE[] = {0x20, 0x02, 0x12, 0x01, 0xA1, 0xD3, 0x0E,
      0xEC, 0x00, 0x58, 0x01, 0xEC, 0x00, 0x03, 0x01, 0x80, 0x01, 0xE0, 0x01,
      0xE0, 0x01};

    if (NFCSTATUS_SUCCESS != phNxpNciHal_send_ext_cmd(sizeof(configPMU_VDDPA_FINE),
                                      configPMU_VDDPA_FINE, &rsp_len, rsp)) {
      NXPLOG_NCIHAL_E("WLC PMU_VDDPA_FINE settings failed");
    }
...
/******************************************************************************
+ * Function         phNxpNciHal_onePoint_calibration
+ *
+ * Description      This function is called to configure One Point Calibration
+ *
+ * Returns          void.
+ *
+ ******************************************************************************/
void phNxpNciHal_onePoint_calibration() {
  uint8_t onePntClbReq = 0;
  uint8_t rsp[PHNCI_MAX_DATA_LEN] = {0};
  uint16_t rsp_len = 0;
  NFCSTATUS status = NFCSTATUS_SUCCESS;
  std::vector<uint8_t> onePointCalibration = {0x2F, 0x3F, 0x01, 0x63};

  if (!GetNxpNumValue(NAME_NXP_WLC_LPCD_ONE_POINT_CALIBRATION,
                      &onePntClbReq,
                      sizeof(onePntClbReq)) || (onePntClbReq > 1)) {
    NXPLOG_NCIHAL_E("One Point calibration config not found or invalid value found");
    return;
  }

  if (onePntClbReq) {
      NXPLOG_NCIHAL_D("One Point Calibration required");

      status = phNxpNciHal_send_ext_cmd(onePointCalibration.size(), onePointCalibration.data(), &rsp_len, rsp);
      if (status != NFCSTATUS_SUCCESS) {
        NXPLOG_NCIHAL_E("One Point Calibration failed");
        return;
      }
  }
}
...
/******************************************************************************
+ * Function         phNxpNciHal_wlc_lpcd_configure
+ *
+ * Description      This function is called to configure LPCD
+ *
+ * Returns          void.
+ *
+ ******************************************************************************/
static void phNxpNciHal_wlc_lpcd_configure() {
  uint8_t isLpcdUpdateReq = 0;
  NFCSTATUS status = NFCSTATUS_SUCCESS;
  uint8_t rsp[PHNCI_MAX_DATA_LEN] = {0};
  uint16_t rsp_len = 0;
  std::vector<uint8_t> getLpcdConfigVal = {0x20, 0x03, 0x03, 0x01, 0xA1, 0xC9};
  std::vector<uint8_t> setLpcdConfigVal = {0x20, 0x02, 0x05, 0x01, 0xA1, 0xC9, 0x01, 0x01};

  if (!GetNxpNumValue(NAME_NXP_WLC_LPCD_EN_DIS_CFG,
                      &isLpcdUpdateReq,
                      sizeof(isLpcdUpdateReq)) || (isLpcdUpdateReq > 1)) {
    NXPLOG_NCIHAL_E("NXP_WLC_LPCD_EN_DIS_CFG config not found or invalid value found");
    return;
  }

  status = phNxpNciHal_send_ext_cmd(getLpcdConfigVal.size(), getLpcdConfigVal.data(), &rsp_len, rsp);
  if (status != NFCSTATUS_SUCCESS) {
    NXPLOG_NCIHAL_E("Failed to get the LPCD_EN_DIS_CFG");
    return;
  }

  if (rsp[3] == NFCSTATUS_SUCCESS && rsp[8] != isLpcdUpdateReq) {
      setLpcdConfigVal[7] = isLpcdUpdateReq;
      status = phNxpNciHal_send_ext_cmd(setLpcdConfigVal.size(), setLpcdConfigVal.data(), &rsp_len, rsp);
      if (status != NFCSTATUS_SUCCESS) {
...
NFCSTATUS phNxpNciHal_resetDefaultSettings(uint8_t fw_update_req,
  ...
  if (status == NFCSTATUS_SUCCESS) {
    ...
    if (IS_CHIP_TYPE_GE(sn100u) && (!IS_CHIP_TYPE_EQ(ctn820))) {
      ret = GetNxpNumValue(NAME_NXP_RDR_DISABLE_ENABLE_LPCD, &num, sizeof(num));
      if (IS_CHIP_TYPE_EQ(sn400u)) return status;
      if (!ret || num == 1 || num == 2) {
...
static NFCSTATUS phNxpNciHal_do_swp_session_reset(void) {
  ...
  if (IS_CHIP_TYPE_GE(sn400u)) {
    static uint8_t reset_swp_session_identity_set[] = {
        0x20, 0x02, 0x0C, 0x01, 0xA0, 0xEA, 0x08, 0xFF,
...
void phNxpNciHal_configFeatureList(uint8_t* init_rsp, uint16_t rsp_len) {
  ...
  if (IS_CHIP_TYPE_GE(sn400u)) is4KFragmentSupported = true;
...
void phNxpNciHal_configureLxDebugMode() {
    NXPLOG_NCIHAL_E("lxDebug not supported");
}
