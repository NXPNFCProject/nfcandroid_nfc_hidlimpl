/******************************************************************************
 *
 *  Copyright 2015-2018,2020-2023, 2025-2026 NXP
 *
...
tNFC_chipType capability::determineChipTypeFromNciRsp(uint8_t* msg,
  ...
  if ((msg_len >= 4) &&
      ((GET_HW_VERSION_NCI_RESP(msg, msg_len) == HW_SN400U) |
       (GET_HW_VERSION_NCI_RESP(msg, msg_len) == HW_SN450U) |
       (GET_HW_VERSION_NCI_RESP(msg, msg_len) == HW_SN470U))) {
    chip_type = sn400u;
    return chip_type;
  }
  if (GET_FW_ROM_VERSION_NCI_RESP(msg, msg_len) ==
    ...
    if (GET_HW_VERSION_NCI_RESP(msg, msg_len) == HW_SN100U_A0 ||
        GET_HW_VERSION_NCI_RESP(msg, msg_len) == HW_SN100U_A2)
      chip_type = sn100u;
    else
      chip_type = sn400u;
  }
  /* PN560 & SN220 have same rom & fw major version but chip type is different*/
    ...
    if ((msg_len >= 4) &&
      ...
      chip_type = pn560;
               ...
               (GET_HW_VERSION_NCI_RESP(msg, msg_len) == HW_CTN820)) {
      chip_type = ctn820;
    } else {
      chip_type = sn220u;
    }
...
tNFC_chipType capability::determineChipTypeFromDLRsp(uint8_t* msg,
  ...
  if ((DnldV2BuilderInstance.getFwRomVersion() ==
           FW_MOBILE_ROM_VERSION_SN400A &&
       DnldV2BuilderInstance.getFwMajorVersion() ==
           FW_MOBILE_MAJOR_NUMBER_SN400A) ||
      DnldV2BuilderInstance.getHwVersion() == HW_SN400U) {
    chip_type = sn400u;
  } else if ((DnldV2BuilderInstance.getFwRomVersion() ==
                  FW_MOBILE_ROM_VERSION_SN400B &&
              DnldV2BuilderInstance.getFwMajorVersion() ==
                  FW_MOBILE_MAJOR_NUMBER_SN400B) ||
             DnldV2BuilderInstance.getHwVersion() == HW_SN450U) {
    chip_type = sn400u;
  } else if ((DnldV2BuilderInstance.getFwRomVersion() ==
                  FW_MOBILE_ROM_VERSION_CTN820 &&
              DnldV2BuilderInstance.getFwMajorVersion() ==
                  FW_MOBILE_MAJOR_NUMBER_CTN820) ||
             DnldV2BuilderInstance.getHwVersion() == HW_CTN820) {
    chip_type = ctn820;
  }
  /* PN560 & SN220 have same rom & fw major version but chip type is different*/
