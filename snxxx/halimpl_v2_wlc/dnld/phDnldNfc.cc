...
NFCSTATUS phDnldNfc_InitImgInfo(bool bMinimalFw, bool degradedFwDnld) {
    ...
    if ((NULL != gpphDnldContext->nxp_nfc_fw) &&
      ...
      if (IS_CHIP_TYPE_GE(sn220u) || IS_CHIP_TYPE_EQ(pn560)) {
        if (IS_CHIP_TYPE_GE(sn400u)) {
          offsetFwMajorNum =
              (static_cast<uint16_t>(gpphDnldContext
                              ->nxp_nfc_fw[EDL_MAJOR_FW_VERSION_INDEX])
               << 8U);
          offsetFwMinorNum =
              (static_cast<uint16_t>(gpphDnldContext
                              ->nxp_nfc_fw[EDL_MINOR_FW_VERSION_INDEX]));
        } else {
          offsetFwMajorNum =
  ...
  if (IS_CHIP_TYPE_GE(sn400u)) {
    DnldV2BuilderInstance.setParams(gpphDnldContext->nxp_i2c_fragment_len,
                                    gpphDnldContext->nxp_nfc_fw,
                                    gpphDnldContext->nxp_nfc_fw_len, wFwVer);
  }
  return wStatus;
}
